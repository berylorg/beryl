...
# Phase 123: Integrate The Range-Backed GPUI Widget Lifecycle (finished)

Accepted and published the typed range-backed multiline widget at
`gpui-text-input` `2dd1d1182dff1f629fdbf55d701bcfdcf0186726`. All 177 tests, locked
checks and rustdoc, canonical and explicit-local single-GPUI dependency graphs, lock-integrity and
resource-release gates, and the independent completion review passed.

# Phase 124: Integrate Settings Window With The Keyed Scrollbar Lifecycle (wip)

Replace `gpui-settings-window`'s removed pre-keyed scrollbar consumption with the accepted exact
owner, mount-generation, visibility-state, render-epoch, snapshot, callback, cancellation, and
release lifecycle for its navigation, content, split-list, and saved-color scroll regions. Preserve
the documented bounded full-render and virtualized strategies, stable scroll handles, focus and
popup behavior, wheel and pointer semantics, and app-neutral ownership; add no compatibility
wrapper or Beryl-specific policy.

Verify unchanged-handle redraw, separately constructed equal-state handles, page and drag routing,
stale geometry, reentrant callbacks, owner replacement, hide, unmount, and window teardown. Run
focused and complete locked `cargo nextest run` suites, locked checks and rustdoc, formatting,
canonical and explicit-local single-GPUI graph and lock-integrity gates, resource cleanup, exact
downstream Beryl pinning, and one fresh independent completion review without sustained stress or
performance measurement.

# Phase 125: Mount Range-Backed Draft And Marker Editing (pending)
...
# Phase 126: Mount Native-Lineage Recovery And Editor Restoration (pending)
...
# Phase 127: Build The Multi-Window Shell And Runtime Bootstrap (pending)
...
# Phase 128: Mount Paged Navigation And Settings (pending)
...
# Phase 129: Mount Main-Window Notices (pending)
...
# Phase 130: Implement Notification Audio Ownership (pending)
...
# Phase 131: Mount Transcript Presentation (pending)
...
# Phase 132: Verify And Close The Useful GUI Checkpoint (pending)
...
# Phase 133: Prove The Exact 0.146.0 Terminal-Repair Surface (pending)
...
# Phase 134: Implement The Private Terminal-Repair Adapter (pending)

Implement the private release-pinned adapter proven by Phase 133: one no-successor-gated
...
# Phase 135: Add Snapshot-Specific Syndic Repair Storage (pending)
...
# Phase 136: Enforce Repair-Required Successor Gates (pending)
...
# Phase 137: Claim Terminal Repair Durably (pending)
...
consumption and backend dispatch unmounted until Phase 147 installs cross-domain repair-media
...
# Phase 138: Integrate Atomic Terminal-Turn Repair (pending)
...
no-successor proof and durable Phase 137 claim, then converges to exactly repaired or explicitly
...
# Phase 139: Add Bounded Outage Capture (pending)
...
# Phase 140: Publish Entirely Fresh Same-Home Recovery (pending)
...
# Phase 141: Verify And Close The Repair And Recovery Checkpoint (pending)
...
gap remains fail-closed: no target dispatches and no durable claim is consumed before Phase 147.

# Phase 142: Implement Branch Discussion Creation (pending)
...
# Phase 143: Implement Resolution Handoff (pending)
...
# Phase 144: Verify And Close Checkpoint 6 (pending)
...
# Phase 145: Implement Asset Admission And Durable Ownership (pending)
...
# Phase 146: Implement Media Rendition Resources (pending)
...
# Phase 147: Implement Generated-Image Repair Media (pending)
...
retain inline base64. After this admission path is installed, mount the Phase 137 claim-consumption
and Phase 138 repair-dispatch path for all eligible targets. Prove crash cuts, collision, orphan
invisibility, coherent publication, and no pre-admission claim consumption.

# Phase 148: Mount Maintenance Presentation And Deferred Cleanup (pending)
...
# Phase 149: Verify And Close Checkpoint 7 (pending)
...
# Phase 150: Reconcile Final Live Authority (pending)
...
# Phase 151: Remove Obsolete Surfaces (pending)
...
# Phase 152: Verify Named Resource Boundaries (pending)
...
# Phase 153: Run End-To-End Functional Verification (pending)
...
# Phase 154: Close The Rework (pending)
