...
- Theme roles expose only the hardcoded supported properties their GUI render sites can consume. Separator roles expose a single `color` property rather than border, foreground, text background, or font properties. Supported role-property value sources can resolve to concrete values from inline concrete values, static parent roles, runtime ambient parent styles, or built-in fallback values. Unsupported properties do not inherit into a role. Runtime ambient inheritance lets embedded styles such as inline code follow the background of final-answer text, user-input text, settings rows, or popups while retaining their own foreground or typography.
- Beryl may expose CAS theme tools that help the model author themes by reading bounded guidance about compact TOML syntax, the role schema tree, static role value resolution, ambient value resolution, transcript/code/settings roles, and troubleshooting. This guidance supplements the structural schema tool rather than replacing it.
...
- The theme editor uses a top-row column navigator over the actual UI role schema tree. Every navigator row is a real UI role id; synthetic grouping labels, folder rows, or other non-role navigator items are invalid.
- Selecting a role changes only the lower property editor, which exposes the hardcoded properties supported by the selected role. Unsupported property entries in installed theme files are ignored when those themes are loaded and are omitted when the theme is later saved.
