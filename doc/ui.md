...
- When an asynchronous operation changes the selected thread, established selected-thread chrome and the transcript region keep rendering the previous coherent state until the replacement thread history is ready to apply. For transcript history, ready means the initial viewport satisfies the transcript feature's presentable-row contract, including completed media readiness or terminal media fallback state. Progress feedback belongs in status, notices, or other localized affordances that do not replace existing labels or content with transient `Opening ...` or loading placeholders.
- Applying a newly selected thread is a single UI transaction: the selected-thread chrome, transcript rows, media-ready row presentation state, and initial transcript viewport state are chosen before the new transcript becomes visible. The transcript renderer may measure rows and reconcile ordinary live layout, but it must not install a second selected-thread-activation scroll position or visible media-readiness update through prepaint, deferred, or post-frame work after the first new-thread frame.
