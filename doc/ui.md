...
- When an asynchronous operation changes the selected thread, established selected-thread chrome and the transcript region keep rendering the previous coherent state until the replacement resident thread history, row presentation model, and initial viewport are ready to apply. Progress feedback belongs in status, notices, or other localized affordances that do not replace existing labels or content with transient `Opening ...` or loading placeholders.
- Applying a newly selected thread is a single UI transaction: the selected-thread chrome, transcript rows, activation-owned row presentation state, and initial transcript viewport state are chosen before the new transcript becomes visible. Markdown parsing, completed media readiness, and media resource admission follow the transcript feature contract as post-publication work with stable row-owned placeholders or terminal fallbacks; they must not become selected-thread publication gates or install a second selected-thread-activation scroll position through prepaint, deferred, or post-frame work after the first new-thread frame.
