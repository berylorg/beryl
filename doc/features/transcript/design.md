...
- When the first stable final-answer content for the active turn becomes available and the user has not manually scrolled, Beryl anchors the start of that final response at the top edge of the transcript viewport exactly once.
...
- Detached manual-scroll state retains bounded virtual trailing runway for the strongest reading anchor from that reading cycle. Before stable final-answer content exists, the retained anchor is the latest user-authored prompt fragment so the user can manually scroll that prompt into the prompt-reread position. Once stable final-answer content exists, the retained anchor is the final-answer start so the user can manually scroll the first final-response line to the top of the transcript viewport. This retained runway is a non-scrolling affordance only; it must not reissue prompt-reread scrolling, commentary following, final-start anchoring, final-growth following, or turn-completion bottom alignment.
...
- Live-turn anchoring is distinct from selected-thread activation. It belongs to accepted new user-authored turns and active response streaming, not to renderer-installed passive anchors for already-loaded history.
