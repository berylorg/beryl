...
- For the current CAS target contract, retained history pages are turn-index pages loaded through `thread/turns/list` with `itemsView = "notLoaded"`.
- Historical turn details also use `thread/turns/list` with `itemsView = "full"` and the smallest known history-page prefix that can contain the highest-priority requested turn. No separate per-turn item-list method is part of the supported transcript contract for this CAS target.
- Skeleton-row visibility is provisional until the requested full details are loaded, sanitized, rendered, and measured. A scheduler pass must treat the current skeleton-derived viewport as a hint, not as proof that every skeleton row in that provisional range should be loaded.
- On existing-thread activation at the transcript tail, missing details are prioritized bottom-up: the latest visible skeleton is requested first, then layout feedback from that loaded turn determines whether earlier skeletons still need details.
- Overscan is a small follow-up target after visible content has been considered. It must not cause Beryl to enqueue the whole skeleton presentation range or load far-off rows simply because short placeholders temporarily fit on screen.
- Queued detail work outside the current scheduler window may be skipped before starting a backend request. User scrolling, thread activation changes, backend target changes, or row remeasurement can invalidate an earlier loading order.
- A full-page detail response may populate multiple returned turns that are still needed by the current viewport, small overscan, or an independent pin, but returned turns outside those bounds must not become retained full details.
- Full historical turn details are retained only while needed for visible-plus-overscan transcript presentation or an independent pin, and are released outside that resident detail window.
- Open turn context menus, active edit targets, loaded media menu targets, and active turns may pin full historical details independently of viewport scheduling. These pins are retention-only unless the owning affordance explicitly starts a detail request; they must not make offscreen rows part of ordinary viewport load priority.
...
- Final-read may adjust the raw row offset only to preserve the same semantic final-answer start when content above that start remeasures, such as media loading, Markdown layout changes, or transcript detail replacement. This correction preserves the original final-start anchor; it is not commentary following, final growth following, or turn-end bottom alignment.
...
- Live row remeasurement, Markdown layout changes, media load/decode, historical detail replacement, and history-page restoration must preserve the active semantic scroll anchor or detached manual position. They must not fall back to raw offsets that can jump into virtual empty space or cause visible oscillation.
- Existing-thread loads without a live-turn anchor open at the real end of the loaded window as their one selected-thread-activation viewport decision and do not synthesize live-turn response runway for already-loaded user input fragments.
- If the latest loaded history turn already contains stable final-answer content, selected-thread activation may expose passive bounded virtual trailing runway for that final-answer start. If the latest loaded row initially renders as a skeleton and full transcript-detail replacement later exposes final-answer content while the transcript is still in selected-thread tail activation, or in manual-detached state whose previous phase was that tail activation, Beryl may refresh that same passive runway. This loaded-history runway uses the same measured final-answer top guard as active final-read placement. It is a manual scroll affordance only: it must not move the activation viewport away from the real tail, reissue final-start anchoring, follow final growth, or use deferred renderer callbacks.
