...
For an active assistant record, the feature supplies the arrival-streaming and durable-reconciling
states without adding a second row, overlay, or progress surface. Arrived bounded fragments that
preserve their normalized CAS text delta identity and order become visible on the next consumed GUI
frame without a typewriter animation; exact Syndic takeover preserves the record's position and
visible prefix.
