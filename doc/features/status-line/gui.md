...
`M/R` presents the model and reasoning readout. Context presents context space, applicable short-window and weekly rate-limit readouts, then `I/IC/O: main: <uncached>/<cached>/<output> sub: <uncached>/<cached>/<output>`. Turn presents the latest turn-state readout. The model/reasoning, Context, and Turn segments retain the feature behavior and interaction availability defined in the status-line design contract.
