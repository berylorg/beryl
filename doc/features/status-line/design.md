...
- The `View` segment is passive status chrome. It consumes transcript-owned viewport, source-turn ownership, and exact-numbering facts that are already present in shell state.
...
- The `total` value is shown only when transcript-owned state proves an exact selected-thread turn total. Beryl must not use the retained transcript-window length as the total when older history may still be unloaded.
...
- The `current` value is the backend turn that owns the last real transcript content line intersecting the transcript viewport. If the viewport bottom is in virtual trailing scroll space, the current value is the final backend turn when at least one turn exists.
