...
Opening closed decision branches uses app-server exact-id archived-thread metadata reads when the backend supports them. After the CAS-live Syndic capture cutover, read-only transcript content for captured closed branches is prepared through the Syndic transcript provider rather than CAS paginated history. Uncaptured closed branches remain explicitly incomplete until a future import or backfill design is approved. Beryl opens a closed decision branch as a read-only transcript with composer disabled.
