...
- Blank or whitespace-only developer-instructions settings contribute no global instruction section to the next turn. Other active feature-owned hidden sections remain independent.
...
- Hidden Beryl instructions supplement normal backend instructions. They must not use collaboration-mode overrides, replace model-catalog instructions, depend on an effective-model lookup, or disappear because a model catalog supplies its own collaboration text.
- The Beryl-maintained app-server fork exposes an optional `turn/start.developerInstructions` string for this purpose. Beryl sends the complete composed hidden context through that field; absence, null, and whitespace-only values supply no turn-specific block. The field is separate from the existing thread-level setting with the same name and does not update thread defaults or Codex configuration.
- CAS snapshots this field with the accepted turn and supplies one separate developer-role block in the effective input of each model request for that turn, alongside normal backend context and before the current user input. Repeated requests within a turn use the same snapshot without accumulating copies. Identical text is accepted and applied again on subsequent turns; it is neither cross-turn deduplicated at admission nor silently truncated. Transport-level previous-response reuse is allowed only when the reconstructed effective input contains exactly the current snapshot; changed or cleared text must invalidate incompatible prefix reuse. Normal request size and context-limit errors remain explicit.
- The turn-specific block is model-input context, not a transcript-visible input item or retained thread instruction override. Clearing the setting prevents injection on subsequent turns and does not rewrite older conversation history. Changes during a running turn take effect on the next turn.
- Capability probing must establish support for this fork-owned field before dispatching a turn containing hidden instructions. Unsupported backends report an explicit compatibility failure before accepting the user draft rather than silently dropping the instructions or falling back to user text or collaboration-mode settings.
- CAS advertises this nonexperimental contract through `initialize.turnScopedDeveloperInstructionsVersion: 1`. Beryl validates that exact version on the managed connection; absent, null, unknown-version, or malformed advertisements cannot establish support. The advertisement requires no synthetic turn or schema-generation subprocess.
