...
- The generated experimental 0.128.0 schema exposes `thread/archive` with required `threadId` and an empty response object, `thread/unarchive` with required `threadId` and a response containing `thread`, `thread/archived` and `thread/unarchived` notifications carrying `threadId`, and a `thread/list.archived` filter where `true` returns archived threads and false or null returns non-archived threads.
- Beryl must treat `thread/archive` as the durable persisted-thread closure primitive when available. `thread/unsubscribe` is subscription/unload cleanup and must not be used as a substitute for closing persisted user-facing decision branches.
