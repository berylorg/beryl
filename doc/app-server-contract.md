...
## Current Target Contract Summary

- The root design invariant targets the CAS 0.137 contract for this Beryl version. These notes record evidence for that target and preserve older observations as historical context.
- Runtime startup must pass a parseable `initialize` userAgent version gate and the same hardcoded required-method probes. For Beryl's CAS 0.137 client identity, live app-server reports its version in the first product token as `beryl/<app-server-version>` and echoes Beryl's client version later in the userAgent string. A newer app-server version string does not enable a different protocol path.
...
- The current transcript-history contract is `thread/turns/list` with `itemsView`; `itemsView = "notLoaded"` is used for retained turn-index pages and bounded `itemsView = "full"` page-prefix requests are used only for visible skeleton detail.
...
- Thread archive surface inspection was performed on May 21, 2026 against `codex-cli 0.128.0` using `codex app-server generate-json-schema --experimental --out <temp-dir>`.
- Archived-thread read/open probing was performed on May 21, 2026 against `codex-cli 0.128.0` over local app-server stdio using an isolated temporary `CODEX_HOME` probe and a disposable persisted forked thread in the normal Codex home. The disposable normal-history probe threads were archived after the probe.
...
- The generated stable 0.128.0 and 0.131.0 schemas expose `fs/watch`, `fs/unwatch`, and `fs/changed`. A May 19, 2026 local stdio probe against both binaries successfully initialized app-server, registered a file watch, edited the watched file, received one `fs/changed` notification containing the changed path, and then unwatched the file.
- `fs/watch` accepts an absolute file or directory path plus a caller-provided connection-scoped `watchId`. `fs/changed` includes the same `watchId` and a list of changed paths. The schema and local source do not expose old contents, new contents, old hashes, new hashes, operation type, durable sequence numbers, reconnect replay, or a workspace identity field.
- Local 0.131.0 source inspection shows app-server registers filesystem watches as non-recursive watches and debounces `fs/changed` notifications by 200 ms before sending sorted changed paths to the owning connection. `fs/unwatch` waits for the watch task to stop so no later notifications are sent for that watch id after the unwatch response. Connection close removes that connection's watch registrations.
- App-server filesystem watch notifications are therefore invalidation signals only. They are not a correctness or synchronization mechanism for graph upkeep, because graph upkeep must handle current source state at the point the AI reads or updates source-backed graph information.
- The generated stable 0.128.0 and 0.131.0 schemas also expose `hooks/list`, `hook/started`, and `hook/completed`. In 0.128.0, the hook event enum includes `preToolUse`, `permissionRequest`, `postToolUse`, `sessionStart`, `userPromptSubmit`, and `stop`; in 0.131.0 it also includes `preCompact` and `postCompact`, and hook metadata adds current-hash and trust-status fields.
- Codex lifecycle hooks are configured user, project, managed, or plugin scripts. The current OpenAI Codex hooks documentation says matching command hooks for one event run concurrently, non-managed hooks require trust review before running, async command hooks and non-command handlers are parsed but not run, and several hook events can add model-visible context.
- Beryl should not use configured Codex hooks as the graph-upkeep trigger or hidden-instruction transport. Beryl should also not use `fs/watch` or `fs/changed` as the graph-upkeep source synchronization path. For graph upkeep, Beryl should use app-server turn lifecycle notifications such as `turn/completed` for post-turn scheduling, current source reads when an AI upkeep pass needs source truth, and Beryl-owned request assembly for hidden graph-upkeep developer instructions.
...
- Historical 0.125 and 0.128 observations showed `thread/read` without top-level model or reasoning-effort fields. Under the current single-contract integration, Beryl treats absent runtime fields on `thread/read` as unknown and consumes only exact fields returned by the target contract; it does not select schema-specific read paths at runtime.
...
- In the generated `codex-cli 0.137.0` stable schema, `thread/turns/list` and `thread/turns/items/list` were absent. In the experimental schema, `thread/turns/list` was present with optional `itemsView` values `notLoaded`, `summary`, and `full`, and `thread/turns/items/list` was present with required `threadId` and `turnId`.
- Live 0.137 probing showed `thread/turns/list itemsView = "notLoaded"` returns turn identity/status/page metadata with `items = []`, while `itemsView = "summary"` returns only user/agent summaries and does not expose generated-image `savedPath`. `itemsView = "full"` returns full turn items, including `imageGeneration.savedPath` and large `imageGeneration.result` payloads.
- Live 0.137 probing showed `thread/turns/items/list` can return JSON-RPC `-32601` with message `thread/turns/items/list is not supported yet` for a real persisted generated-image turn even though the method appears in the experimental schema and Beryl initialized with `experimentalApi = true`.
- For the current CAS 0.137 target, Beryl uses `thread/turns/list itemsView = "notLoaded"` as the cheapest turn-index view and uses bounded `thread/turns/list itemsView = "full"` requests for visible history skeleton details. Beryl does not use the schema-exposed per-turn item-list method in this target because live 0.137 reports it as runtime unsupported.
- For correctness-sensitive composer image-label allocation, Beryl treats thread ids, turn ids, and `updatedAt` as backend-owned observations, not sufficient authority by themselves. A GUI-local label cache can only unblock image paste after Beryl validates the cache's recorded history frontier against current backend turn pages, preferably through `thread/turns/list itemsView = "notLoaded"` so generated image bytes are not loaded.
- If a skeleton-mode page returns item vectors despite requesting `notLoaded`, Beryl discards those item vectors before retaining the page so whole-page detail data cannot bypass the visible-detail cache.
- The 0.137 bounded full-page detail path preserves Beryl's app-side retention bounds, because Beryl sanitizes the response stream, keeps only the requested turn's full item vector in visible-plus-overscan transcript state, and releases it outside the resident detail window. It cannot prevent app-server itself from assembling and sending generated-image result bytes for that full page prefix.
...
- Historical 0.128 experimental schema inspection showed `thread/start.dynamicTools` with `name`, `description`, `inputSchema`, optional `namespace`, and optional `deferLoading`. Beryl-owned dynamic tools that need to be available during a conversation thread are registered when Beryl creates the thread; a later CAS migration must update the root design invariant and these notes before relying on another registration surface.
- Historical 0.128 schema inspection showed per-turn developer instructions through `turn/start.collaborationMode.settings.developer_instructions`, with `collaborationMode.mode` set to `default` or `plan` and `settings.model` required. `thread/start.developerInstructions` remained available for new threads in that observation.
...
- Historical 0.128 experimental schema inspection exposed `thread/archive`, `thread/unarchive`, `thread/archived`, `thread/unarchived`, and a `thread/list.archived` filter.
- When the current target reports archive support, Beryl treats `thread/archive` as the durable persisted-thread closure primitive. `thread/unsubscribe` is subscription/unload cleanup and must not be used as a substitute for closing persisted user-facing decision branches.
- Live 0.128.0 probing showed `thread/archive` succeeds on a loaded and subscribed persisted fork, emits `thread/status/changed` followed by `thread/archived`, unloads the target, removes it from `thread/list archived=false`, and makes it appear in `thread/list archived=true`. The same archived-list behavior holds from a fresh app-server connection.
- `thread/read` by exact id succeeds on archived persisted threads without `thread/unarchive`. `includeTurns=false` returns metadata with `status.type = "notLoaded"`, and `includeTurns=true` returns historical turns and items. The archived read preserved fork metadata in the observed `forkedFromId` field.
- `thread/turns/list` by exact id succeeds on archived persisted threads without `thread/unarchive` and returns turn pages.
- `thread/resume excludeTurns=true` succeeds on an archived persisted thread and emits `thread/status/changed`, but the probe still found the thread only in `thread/list archived=true` afterward. Resume therefore loads the archived thread for the client connection but does not itself unarchive it. A GUI that uses resume for closed-branch inspection must still force read-only presentation and must not enable composer or ordinary writable activation from the returned idle status alone.
- `thread/unarchive` is explicit. The live probe emitted `thread/unarchived`, moved the thread back to `thread/list archived=false`, and removed it from `thread/list archived=true`; cleanup re-archived the probe afterward. Beryl must not call `thread/unarchive` for ordinary closed decision-branch activation.
- The live `thread/unarchive` response for a forked archived probe returned a `thread` whose `forkedFromId` was null even though archived `thread/read` for the same id had returned the durable parent id. Callers that need lineage after unarchive should still prefer `thread/read` enrichment over trusting list or unarchive response metadata.
- A cleanup re-archive request for a thread already present in `thread/list archived=true` returned JSON-RPC error `-32600` with message `no rollout found for thread id ...` in the live probe. Callers should not assume `thread/archive` is idempotent for already-archived threads; retry paths need to tolerate exact evidence that the thread is already archived.
- An isolated temporary `CODEX_HOME` shell-command-only probe remained readable by exact `thread/read` and `thread/turns/list` after archive but did not appear in either archived or non-archived `thread/list` results. The list-filter conclusions above are based on a normal persisted forked thread with a real turn in the normal Codex home.
...
- Historical 0.128 schema inspection showed `thread/fork` with required `threadId` and optional runtime or configuration overrides; the experimental schema additionally exposed path-based fork and richer history-persistence fields.
...
- Historical 0.128 `Thread` schemas included optional `forkedFromId`, described as the source thread id when a thread was created by forking another thread. The field appeared in thread list/read/fork/rollback/resume response shapes and the thread-started notification shape.
...
- Historical 0.128 schema inspection showed `thread/rollback` with required `threadId` and `numTurns`, where `numTurns` is the number of trailing user turns to drop and must be at least 1. It was not targeted by a turn id.
...
- A WSL-launched app-server needs a Linux path that resolves inside that selected distro. For a Beryl image asset stored under the Windows user profile, the practical backend path is the selected distro's mounted view of that file, such as `/mnt/c/Users/<user>/.beryl/...`. Beryl's Windows GUI process validates the source asset through its original Windows path before submitting the mapped backend path; it must not validate Windows drive assets through `\\wsl.localhost\<distro>\mnt\<drive>\...`.
...
- Beryl must not treat the relative ordering of `thread/status/changed idle` and `turn/completed` as a correctness boundary. If a worker needs terminal proof for a known turn id and sees the target thread become idle before seeing `turn/completed`, it must use an exact thread read/history proof before declaring success or failure.
...
- Historical 0.128 experimental schema inspection declared reasoning lifecycle items, `item/reasoning/summaryPartAdded`, `item/reasoning/summaryTextDelta`, `item/reasoning/textDelta`, completed reasoning `summary` arrays, and token usage with `reasoningOutputTokens`. Live probes observed reasoning item start/completion without summary text for some turns and summary text deltas for other turns.
- Current Activity panel presentation treats reasoning summary text as optional activity detail. Beryl shows `reasoning` when only lifecycle is available and `reasoning: <summary>` only from summary text fields. Raw `item/reasoning/textDelta` content is not a user-facing activity detail.
...
- In 0.137 history probing, generated-image `savedPath` was present only in full turn item data. Summary and not-loaded turn views were insufficient for historical generated-image rendering because they did not carry the native image item record.
- Beryl's history sanitizer strips generated-image `result` fields when a `savedPath` is present, so transcript history can retain the path needed for source-backed rendering without retaining base64 image bytes in typed app state.
