...
- For the current CAS 0.137 target, Beryl uses `thread/turns/list itemsView = "notLoaded"` as the cheapest non-rendered turn-index view and uses bounded `thread/turns/list itemsView = "full"` requests for resident full-detail transcript windows. Beryl does not use the schema-exposed per-turn item-list method in this target because live 0.137 reports it as runtime unsupported.
...
- If a `notLoaded` turn-index page returns item vectors despite the request, Beryl discards those item vectors before retaining the index page so whole-page detail data cannot bypass transcript residency policy.
- The 0.137 bounded full-page detail path preserves Beryl's app-side retention bounds, because Beryl sanitizes the response stream, keeps only full item vectors admitted by the current transcript residency policy or explicit pins, and releases them outside the resident full-detail window. It cannot prevent app-server itself from assembling and sending generated-image result bytes for that full page request.
