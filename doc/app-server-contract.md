...
It is an implementation aid, not a design authority. Relevant feature, system, and package design documents remain authoritative; `crates/beryl-backend/doc/design.md` owns Beryl's backend protocol boundary.
...
- The observed response includes `userAgent`, `codexHome`, `platformFamily`, and `platformOs`. On exact 0.146.0, `userAgent` combines the client project name with the CAS version and initialize `clientInfo`; a Beryl client produced a `beryl/0.146.0 ... (beryl; 0.1.0)` value. A target proof may validate the `beryl/0.146.0` CAS-version prefix, but this field is not the CLI `codex --version` banner and must not be validated by equality with `codex-cli 0.146.0`.
...
- In the inspected 0.146.0 experimental schema, each dynamic-tool specification is a tagged union. Function tools require `type = "function"`; namespace tools require `type = "namespace"` and contain nested tagged function tools. Beryl keeps logical function specifications flat for app-side registration and normalizes them privately at `thread/start`: unnamespaced functions become tagged top-level functions, while functions sharing a namespace become nested tagged functions under one deterministic namespace entry.
...
- The inspected 0.128.0 and 0.146.0 schemas do not state whether `thread/fork` copies a source thread's registered dynamic tools, and Beryl cannot re-register those tools through `thread/fork`, `thread/rollback`, `thread/resume`, or `turn/start`. A targeted guarded live proof against exact 0.146.0 established that a registered namespaced Beryl tool survives populated root fork, full user-turn rollback, archive, unarchive, resume, and a later child turn while retaining exact root lineage, empty child history, required model-provider identity, and nonconflicting optional runtime metadata. This exact-target evidence supports repeated lifecycle `yield` calls from prepared phase children; mocks or a successful fork alone would not have been sufficient.
...
- `thread/unsubscribe` accepts `threadId` and returns status `notLoaded`, `notSubscribed`, or `unsubscribed`; it removes only that connection's subscription to thread turn and item events.
- On the exact 0.146.0 target, removing the last subscriber does not unload the thread immediately. App-server keeps it loaded until it has no subscribers and no activity for 30 minutes, then runs session-end hooks and emits `thread/closed`. A bounded caller must not treat `unsubscribed`, or a short wait after it, as proof of unload.
- `thread/archive` accepts `threadId`, moves the persistent rollout into archived storage, attempts the same for spawned descendants, returns `{}`, and emits `thread/archived` for each archived thread. Archive is an explicit persistence lifecycle operation, not subscription cleanup or physical deletion.
- `thread/unarchive` accepts `threadId`, restores an archived rollout to active session storage, returns the restored `thread`, and emits `thread/unarchived`.
- `thread/delete` accepts `threadId`, physically deletes an active or archived persistent thread and its spawned descendants, returns `{}`, and emits `thread/deleted` for each deleted thread. It is not valid for ephemeral threads.
- For title generation, Beryl should create one fresh ephemeral thread per title attempt and call `thread/unsubscribe` after the attempt reaches a terminal state. This releases Beryl's subscription to an ephemeral loaded thread; it is not immediate unload or deletion of persisted conversation history.
...
- The inspected `thread/fork` contract has no caller-supplied idempotency key, transaction handle, or request identity that can later be resolved to the created child. If the request commits but timeout or transport loss prevents its response from reaching Beryl, the child identity is indeterminate; thread inventory comparison cannot recover it authoritatively.
- Beryl must report that ambiguous result as a possible unidentified backend child, keep the source selected, and refresh inventory without guessing or automatically selecting a candidate. Only a returned exact child id may drive rollback, deletion, registration, or activation.
...
- `excludeTurns = true` changes only the fork response payload. It does not erase inherited effective history and must not be used as a clean-context substitute for rollback.
...
- Forked-and-rolled-back responses retain the backend fork-parent field in the protocol shapes inspected for 0.128.0. Clean phase-thread preparation must still verify both empty effective history and the expected `forkedFromId` before exposing a child to GUI selection.
