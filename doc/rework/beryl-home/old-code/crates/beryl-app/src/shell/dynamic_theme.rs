...
impl ShellView {
    pub(super) fn handle_beryl_theme_dynamic_tool_shell_request(
        ...
        match parsed {
            ...
            ThemeDynamicToolRequest::Activate { .. } => {
                request.respond(theme_tool_failure_response(
                    request.request(),
                    "theme_settings_unavailable",
                    "Theme activation awaits the typed Beryl settings service.",
                ))
            }
        }
    }
