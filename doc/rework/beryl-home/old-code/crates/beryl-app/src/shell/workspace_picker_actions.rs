...
use crate::{
    BerylWorkspacePersistence, StartupPersistence, WorkspaceGraphUpkeepPolicy, WorkspaceUiState,
};
use tracing::warn;
...
fn delete_workspace_for_picker(
    ...
    let replacement_graph_upkeep_policy =
        if let Some(replacement_workspace) = replacement_workspace.as_ref() {
            Some(
                workspace_persistence
                    .load_workspace_graph_upkeep_policy(replacement_workspace.id())
                    .map_err(|error| error.to_string())?,
            )
        } else {
            None
