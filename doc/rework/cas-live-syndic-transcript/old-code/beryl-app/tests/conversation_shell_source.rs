...
fn workspace_shell_rendering_uses_initialized_controls_and_shared_composer_frame() {
    ...
    assert!(measure_composer_body.contains("enabled,\n        false,"));
...
#[test]
fn visible_transcript_panel_is_new_resident_host_boundary() {
    ...
    assert!(
        conversation_source.contains("use crate::shell::syndic_transcript::SyndicTranscriptPanel;")
    );
    assert!(conversation_source.contains("transcript_panel: &Entity<SyndicTranscriptPanel>"));
    assert!(conversation_source.contains("AnyView::from(transcript_panel.clone())"));
    assert!(panel_source.contains("host: SyndicTranscriptHost"));
    assert!(panel_source.contains("SyndicTranscriptHost::empty()"));
}

#[test]
fn transcript_scroll_commands_route_to_resident_host_boundary() {
    let shell_source = include_str!("../src/shell.rs");
    let panel_source = include_str!("../src/shell/syndic_transcript/panel.rs");
    let gui_control_body = rust_function_body(
        shell_source,
    ...
    assert!(shell_source.contains("DiagnosticChildCommand::ScrollTranscript"));
    assert!(shell_source.contains("handle_scroll_transcript_tool_result(arguments, window, cx)"));
    assert!(gui_control_body.contains("GuiControlToolRequest::ScrollTranscript(arguments)"));
    assert!(
        gui_control_body.contains("handle_scroll_transcript_tool_result(arguments, window, cx)")
    );
    ...
    assert!(scroll_body.contains("window.viewport_size().height"));
    assert!(!scroll_body.contains("apply_transcript_wheel_command"));
    assert!(!scroll_body.contains("TranscriptViewportState"));
...
#[test]
fn selected_thread_activation_starts_resident_host_boundary() {
    let shell_source = include_str!("../src/shell.rs");
    ...
    let host_activation_body = rust_function_body(
        shell_source,
        "fn begin_transcript_host_activation_for_thread",
    );
    let optional_activation_body = rust_function_body(
        shell_source,
        "fn begin_transcript_host_activation_for_optional_thread",
    );
    let unavailable_activation_body = rust_function_body(
        shell_source,
        "fn begin_unavailable_transcript_host_activation",
    );
    let selected_source_body = rust_function_body(
        shell_source,
        "fn transcript_activation_source_for_selected_thread",
    );
    let view_id_body = rust_function_body(shell_source, "fn transcript_view_id_for_thread");
    ...
    let worker_finish_body = rust_function_body(
        lifecycle_source,
        "pub(super) fn finish_thread_activation_worker",
    );
    let decision_activation_body = rust_function_body(
        decision_resolution_source,
        "fn begin_decision_resolution_parent_activation",
    );

    assert!(panel_source.contains("pub(crate) fn begin_activation("));
    assert!(panel_source.contains("self.host.begin_activation(seed)"));
    assert!(!panel_source.contains("pub(crate) fn host_mut("));

    assert!(host_activation_body.contains("TranscriptActivationSeed::new"));
    assert!(view_id_body.contains("TranscriptViewId(thread_id.to_string())"));
    assert!(host_activation_body.contains("panel.begin_activation(seed)"));
    assert!(!host_activation_body.contains("ThreadInfo"));
    assert!(!host_activation_body.contains(".turns"));
    assert!(optional_activation_body.contains("begin_unavailable_transcript_host_activation"));
    assert!(unavailable_activation_body.contains("TranscriptActivationSeed::unavailable"));

    assert!(selected_source_body.contains("SelectedThreadActivationSource::StartupRestore"));
    assert!(selected_source_body.contains("SelectedThreadActivationSource::BackendReopenRefresh"));
    assert!(selected_source_body.contains("SelectedThreadActivationSource::Explicit(source)"));

    assert!(
        selector_activation_body
            .contains("let transcript_activation_source =\n            Self::transcript_activation_source_for_thread_navigation(source);")
    );
    assert!(selector_activation_body.contains("begin_transcript_host_activation_for_thread"));
    assert!(selector_activation_body.contains("WorkspaceOpenIntent::ThreadSelectorActivation"));
    let selector_same_target_tail = selector_activation_body
        .split("let activation_ui_started")
        .nth(1)
        .expect("selector activation should have a same-target worker branch");
    assert_order(
        selector_same_target_tail,
    ...
    assert_order(
        selector_same_target_tail,
        "self.begin_transcript_host_activation_for_thread(",
        "spawn_thread_activation_worker(",
    );

    assert!(graph_activation_body.contains("TranscriptActivationSource::ThreadGraph"));
    assert!(graph_activation_body.contains("begin_transcript_host_activation_for_thread"));
    let graph_same_target_tail = graph_activation_body
        .split("let activation_ui_started")
        .nth(1)
        .expect("graph activation should have a same-target worker branch");
    assert_order(
        graph_same_target_tail,
        "surface.begin_thread_activation(",
        "self.begin_transcript_host_activation_for_thread(",
    );
    assert_order(
    ...
    assert!(decision_activation_body.contains("surface.begin_thread_activation("));
    assert!(decision_activation_body.contains("begin_transcript_host_activation_for_thread"));
    assert!(decision_activation_body.contains("TranscriptActivationSource::ThreadGraph"));
    assert_order(
        decision_activation_body,
        "surface.begin_thread_activation(",
        "self.begin_transcript_host_activation_for_thread(",
    );

    assert!(finish_publication_body.contains("begin_transcript_host_activation_for_thread"));
    assert!(finish_publication_body.contains("transcript_activation_source_for_selected_thread"));
    assert!(!finish_publication_body.contains(".turns"));

    assert!(
        workspace_finish_body
            .contains("finish_published_selected_thread_activation(publication, cx)")
    );
    assert!(workspace_finish_body.contains("begin_transcript_host_activation_for_optional_thread"));
    assert!(workspace_finish_body.contains("TranscriptActivationSource::BackendReopen"));
    assert!(workspace_finish_body.contains("TranscriptActivationSource::StartupRestore"));
    assert!(
        worker_finish_body.contains("finish_published_selected_thread_activation(publication, cx)")
    );
}
...
#[test]
fn selected_thread_activation_worker_does_not_prepare_legacy_image_resolver() {
    ...
    assert!(activation_worker_body.contains("ThreadActivationLoader::load_existing_thread"));
    assert!(activation_worker_body.contains("ThreadActivationOutcome::Activated"));
    assert!(
        !activation_worker_body.contains("transcript_image_path_resolver_for_workspace_assets")
    );
    assert!(!activation_worker_body.contains("TranscriptImagePathResolver::default"));
    assert!(!activation_worker_body.contains("transcript_image_path_resolver_for_turns"));
    assert!(!activation_worker_body.contains("image_resolver"));
}
...
fn selected_thread_publication_is_not_gated_by_renderer_media_admission() {
    ...
    assert!(publish_activation_body.contains("true"));
    assert!(!publish_activation_body.contains("presentability"));
    ...
    assert!(!progress_cap_body.contains("presentability"));
    assert!(!progress_cap_body.contains("preparation_progress"));
    assert!(!progress_cap_body.contains("media_progress"));
}

#[test]
fn transcript_focus_uses_syndic_panel_context_without_legacy_bindings() {
    let shell_source = include_str!("../src/shell.rs");
    let panel_source = include_str!("../src/shell/syndic_transcript/panel.rs");
    let legacy_bind_keys = concat!("render", "::", "transcript", "::", "bind_keys(cx)");

    assert!(!shell_source.contains(legacy_bind_keys));
    assert!(panel_source.contains("const SYNDIC_TRANSCRIPT_KEY_CONTEXT"));
    assert!(panel_source.contains("\"SyndicTranscriptPanel\""));
    assert!(panel_source.contains("impl Focusable for SyndicTranscriptPanel"));
    assert!(panel_source.contains(".key_context(SYNDIC_TRANSCRIPT_KEY_CONTEXT)"));
    assert!(panel_source.contains(".track_focus(&self.focus_handle)"));
}
...
#[test]
fn composer_image_label_sync_does_not_wait_for_retired_history_page_loading() {
    let shell_source = include_str!("../src/shell.rs");
    let sync_source = include_str!("../src/shell/composer_image_label_sync.rs");
    ...
    assert!(!poll_body.contains("poll_thread_history_page_updates"));
    assert_order(
        poll_body,
...
fn decision_branch_publication_validates_registration_and_binding_before_graph_persistence() {
    ...
    assert!(finish_body.contains("register_decision_branch_thread"));
    ...
    assert_order(
        finish_body,
        "register_decision_branch_thread",
