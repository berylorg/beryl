...
fn thread_title_update_row(
    shell: &ShellRenderFrame<'_>,
    entry: TranscriptThreadTitleUpdateMenuEntry,
    cx: &mut Context<ShellView>,
) -> AnyElement {
    if let Some(reason) = entry.disabled_reason() {
        return disabled_thread_title_update_row(shell, reason).into_any_element();
    }

    secondary_button(
        shell,
        "transcript-update-thread-title-row",
        "Update thread title",
        cx.listener(ShellView::update_thread_title_from_menu),
    )
    .into_any_element()
}

fn disabled_thread_title_update_row(
    shell: &ShellRenderFrame<'_>,
