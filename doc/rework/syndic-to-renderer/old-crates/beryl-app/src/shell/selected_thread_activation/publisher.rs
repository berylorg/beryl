...
impl SelectedThreadPublisher {
    ...
    fn publish(
        ...
        let history_generated_image_count = staged
            ...
            .count();
        publish_selected_thread_shell_state(surface, &staged.thread);
...
fn publish_selected_thread_shell_state(
    surface: &mut ConversationSurfaceState,
    thread: &beryl_backend::ThreadInfo,
) {
    let summary = thread.summary();
    surface.upsert_selected_thread(summary.clone());
    ...
    surface.execution_details.reset();
    surface.transcript_presentation.clear();
    surface.transcript_live_scroll.clear_inactive();
    surface.transcript_user_scrolled = false;
    surface.transcript_history_window = Default::default();
    ...
    surface.last_transcript_content_scroll_signature = None;
    surface.transcript_residency_frame_facts = None;
    surface.transcript_navigation_frame_snapshot = None;
    surface.transcript_streamed_navigation_snapshot = None;
    surface.transcript_event_time_scroll.clear();
    ...
    surface.transcript_list_state.reset(0);
...
pub(super) fn publish_legacy_history_window(
    surface: &mut ConversationSurfaceState,
    thread: &beryl_backend::ThreadInfo,
