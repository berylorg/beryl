...
use beryl_backend::{
    ...
    ManagedBackendError, ManagedBackendSession, StopAttemptCorrelation, StopOperationCorrelation,
    TurnInterruptDisposition, TurnInterruptOutcome, VolatileInterruptAdmissionFailure,
    VolatileInterruptAuthorization, VolatileInterruptCorrelation, VolatileInterruptOutcome,
};

use crate::{
    support::{authorize_volatile, connect_initialized, connect_initialized_unbound, target},
    websocket::{
...
fn volatile_authority_is_nominally_separate_from_durable_stop() {
    ...
    let _: fn(
        &mut ManagedBackendSession,
        VolatileInterruptAuthorization,
        Duration,
    ) -> VolatileInterruptOutcome = ManagedBackendSession::interrupt_volatile;

    let source = include_str!("../../src/exact_interruption.rs");
    for forbidden in ["HomeStore", "FailureGeneration", "syndic", "gpui"] {
        assert!(
            !source.contains(forbidden),
            "volatile backend authority crossed forbidden boundary with {forbidden}"
        );
    }
    fn assert_closed_admission_failure(failure: VolatileInterruptAdmissionFailure) {
        match failure {
            VolatileInterruptAdmissionFailure::FailedBeforeWriter
            | VolatileInterruptAdmissionFailure::WriterReturnedNotCommitted => {}
        }
    }
    assert_closed_admission_failure(VolatileInterruptAdmissionFailure::FailedBeforeWriter);
    assert_closed_admission_failure(VolatileInterruptAdmissionFailure::WriterReturnedNotCommitted);
}
...
fn volatile_wire_matches_pinned_interrupt_and_omits_correlation() {
    ...
    let authorization = authorize_volatile(&mut session);
    let outcome = session.interrupt_volatile(authorization, TIMEOUT);
...
fn volatile_possible_dispatch_retires_and_revocation_refuses_before_bytes() {
    ...
    assert!(matches!(
        session.authorize_volatile_interrupt(
            target(),
            VolatileInterruptAdmissionFailure::FailedBeforeWriter,
            VolatileInterruptCorrelation::from_bytes([1; 16]),
...
fn volatile_authorization_rejects_unbound_and_foreign_targets() {
    ...
    assert!(matches!(
        session.authorize_volatile_interrupt(
            target(),
            VolatileInterruptAdmissionFailure::FailedBeforeWriter,
            VolatileInterruptCorrelation::from_bytes([2; 16]),
            CallerNoSuccessorFence::issue(),
        ),
    ...
    assert!(matches!(
        session.authorize_volatile_interrupt(
            foreign,
            VolatileInterruptAdmissionFailure::WriterReturnedNotCommitted,
            VolatileInterruptCorrelation::from_bytes([3; 16]),
            CallerNoSuccessorFence::issue(),
        ),
