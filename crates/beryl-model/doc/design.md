...
- Workspace-conversation state owns the default runtime environment, runtime-bound explicit workspace members, primary-member designation, active-thread selection, backend thread-name snapshots, manual GUI-local thread title metadata, whether a registered backend thread was created by Beryl, automatic thread-title generation attempt state for Beryl-created threads, optional lifecycle orchestration-root identity, thread/member binding metadata, last-known exact per-thread token-usage snapshots for status presentation, and registered thread summaries for one semantic workspace.
- A lifecycle orchestration root records its own backend thread id as orchestration-root identity, and every phase child generated for that sequence records the same id. This provenance is scoped per registered thread so one workspace can contain multiple independent phase sequences.
- Lifecycle orchestration-root identity controls only later phase-child preparation. It must not synthesize, replace, or override backend fork-parent metadata used for thread lineage presentation, and its absence remains valid for threads outside a lifecycle phase sequence.
