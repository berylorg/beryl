use beryl_home_store::{DomainReader, DomainValidator, HomeStore, ValidationContribution};
use beryl_model::{DomainRevision, ExecutionBinding, SyndicDraftId, SyndicThreadId};

use crate::{
    DraftByThreadRecord, DraftEditHistoryFrontierV1, DraftPieceRootRecordV1, DraftRecord,
    DraftSubmissionIntent, HistorySummaryRecord, InputGateRecord, InputGateState,
    ...
    ThreadRecord,
    codec::{
        DraftByThreadFamily, DraftsFamily, Family, HistorySummariesFamily, InputGatesFamily,
        ThreadAttributesFamily, ThreadCatalogSummariesFamily, ThreadExecutionsFamily,
        ThreadsFamily, family_point_limit,
    },
    domain::SyndicDomain,
    draft_piece::{
...
impl DomainValidator<SyndicDomain> for ValidatePristineThread {
    ...
    fn validate(&self, reader: &DomainReader<'_, SyndicDomain>) -> Result<(), Self::Error> {
        ...
        if mutation_required::<ThreadsFamily>(reader, &thread_id)? != facts.thread
            || mutation_required::<DraftByThreadFamily>(reader, &thread_id)? != facts.index
            || mutation_required::<DraftsFamily>(reader, &facts.draft.id())? != facts.draft
            || mutation_required::<DraftPieceRootsFamily>(reader, &facts.draft.piece_root().key())?
                != facts.root
            || mutation_required::<DraftEditHistoryFrontiersFamily>(
                reader,
                &facts.draft.history().key(),
            )? != facts.history
            || mutation_required::<ThreadExecutionsFamily>(reader, &thread_id)? != facts.execution
            || mutation_required::<ThreadAttributesFamily>(reader, &thread_id)? != facts.attributes
            || mutation_required::<HistorySummariesFamily>(reader, &thread_id)? != facts.summary
            || mutation_required::<ThreadCatalogSummariesFamily>(reader, &thread_id)?
                != facts.catalog
            || mutation_required::<InputGatesFamily>(reader, &thread_id)? != facts.input_gate
            || validate_closure(facts).is_err()
