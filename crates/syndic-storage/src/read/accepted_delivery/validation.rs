...
pub(super) const fn is_ready(lifecycle: AcceptedInputLifecycle) -> bool {
    matches!(
        lifecycle,
...
pub(super) fn validate_ready_route(
    input: &AcceptedInputRecord,
    leaf: &AcceptedRouteLeafRecord,
...
fn route_disagreement() -> SyndicReadError {
    SyndicReadError::Invariant("ready accepted-input route relationships disagree")
}

fn execution_disagreement() -> SyndicReadError {
    SyndicReadError::Invariant("ready steering execution relationships disagree")
}
