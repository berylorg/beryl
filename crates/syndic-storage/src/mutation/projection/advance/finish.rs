...
impl AdvanceBuildRecords {
    ...
    pub(super) fn validate_collisions(
        ...
        for projection in &self.projections {
            if point::<ProjectionsFamily>(reader, &projection.id())?
                .is_some_and(|stored| stored != *projection)
            {
                return Err(SyndicMutationError::ProjectionIdentityCollision);
            }
        }
        for index in &self.stable_projection_indexes {
        ...
        for index in &self.projection_indexes {
        ...
        for (resource, index) in self.resources.iter().zip(&self.resource_indexes) {
            if point::<ResourcesFamily>(reader, &resource.id())?
                .is_some_and(|stored| stored != *resource)
                || point::<ProjectionResourcesFamily>(
                    reader,
                ...
                .is_some_and(|stored| stored != *index)
            {
                return Err(SyndicMutationError::ProjectionIdentityCollision);
            }
        }
