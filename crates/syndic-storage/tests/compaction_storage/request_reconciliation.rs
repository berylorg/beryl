...
fn live_request_publication_reconciles_prior_then_exact() {
    let fixture = CompactionFixture::new("live-request", 125);
...
fn late_matching_acknowledgement_reconciles_without_mutating_terminal_successor() {
    let (fixture, id) = settled_fixture("late-ack", 130);
...
fn late_same_attempt_completion_unknown_preserves_terminal_successor() {
    let (fixture, id) = settled_fixture("late-unknown", 140);
    let before = fixture.operation(id);
    let request = request(
        &fixture,
        id,
...
fn late_acknowledgement_survives_user_work_settlement() {
    let fixture = CompactionFixture::new("late-ack-progressed-gate", 145);
...
fn terminal_successor_rejects_contradictory_dispositions_and_attempts() {
    let (fixture, id) = settled_fixture("late-contradiction", 150);
    let operation = fixture.operation(id);
    for disposition in [
