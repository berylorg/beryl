...
fn receipt_rejects_wrong_binding_provenance() {
    let (fixture, id) = continuation_fixture("continuation-binding-provenance", 115);
    let receipt = fixture
        .storage
...
fn receipt_rejects_wrong_parent_provenance() {
    let (fixture, id) = continuation_fixture("continuation-parent-provenance", 116);
    let receipt = fixture
        .storage
        .compaction_settlement_receipt(
            &fixture.store,
...
fn rejects_forged_continuation_path_revision() {
    let (fixture, id) = continuation_fixture("continuation-coherent-topology", 119);
...
fn identities_are_rederived_from_durable_home_authority() {
    let (fixture, id) = continuation_fixture("continuation-derived-identity", 117);
    let operation = fixture.operation(id);
...
fn rejects_cross_thread_origin_and_forged_pending_lifecycle() {
    let (fixture, id) = continuation_fixture("continuation-origin-lifecycle", 118);
    let receipt = fixture
        .storage
        .compaction_settlement_receipt(
            &fixture.store,
