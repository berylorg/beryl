...
fn empty_active_route_returns_an_exact_safe_admission_source() {
    let fixture = active_stop_fixture("stop-admission-empty");
...
fn path_neutral_accepted_input_advance_returns_the_current_gate_and_route() {
    let fixture = active_stop_fixture("stop-admission-path-neutral");
...
fn a_delivering_steering_leaf_is_typed_ineligible() {
    let fixture = active_stop_fixture("stop-admission-delivering");
...
fn admitted_joined_and_claimed_stops_return_one_shared_live_authority() {
    let fixture = active_stop_fixture("stop-admission-live-descendants");
    fixture.admit_stop();
...
fn idle_and_pending_gates_are_typed_ineligible() {
    ...
    let pending = pending_turn_fixture("stop-admission-pending");
...
fn awaiting_terminal_is_typed_ineligible_without_stop_authority() {
    let fixture = active_stop_fixture("stop-admission-awaiting-terminal");
    let state = fixture
        .storage
        .turn_state(&fixture.store, fixture.turn, point_limit())
        .unwrap()
...
fn compacting_and_finalizing_gates_are_typed_ineligible() {
    let compacting = recovery_support::pending_home("stop-admission-compacting", 680);
    ...
    let finalizing =
        finalizing_history_support::terminal_home("stop-admission-finalizing", 681, false);
...
fn a_stale_binding_successor_is_typed_ineligible() {
    let fixture = active_stop_fixture("stop-admission-stale-binding");
    ...
    let abandonment = active
        .generic_abandonment(" deliberate target loss", active.minimum_timestamp())
        .unwrap();
    match fixture
        .store
...
fn a_constituent_point_read_budget_exhaustion_is_returned() {
    let fixture = active_stop_fixture("stop-admission-point-limit");
...
fn a_stop_revision_change_between_complete_passes_is_concurrent_change() {
    ...
    let fixture = active_stop_fixture_with_faults("stop-admission-concurrent-stop", faults.clone());
    fixture.admit_stop();
...
fn matching_terminal_consumption_during_the_second_pass_is_concurrent_change() {
    ...
    let faults = FaultController::new();
    let fixture = active_stop_fixture_with_faults("stop-admission-terminal-race", faults.clone());
    fixture.admit_stop();
    correlate_user_item(
        &fixture.store,
        &fixture.storage,
...
fn a_gate_route_change_inside_one_raw_pass_is_concurrent_change() {
    ...
    let faults = FaultController::new();
    let fixture =
        active_stop_fixture_with_faults("stop-admission-concurrent-route", faults.clone());
    let blocks = (0..8)
        .map(|_| faults.block_next(FaultPoint::BeforeReadConfirmation))
        .collect::<Vec<_>>();
...
fn a_stably_missing_active_cas_turn_is_an_invariant_failure() {
    ...
    let fixture = active_stop_fixture("stop-admission-missing-cas-turn");
...
fn a_missing_reverse_cas_turn_authority_is_an_invariant_failure() {
    let fixture = active_stop_fixture("stop-admission-missing-cas-turn-index");
    delete_fixture(
        &fixture.store,
        &fixture.storage,
...
fn a_present_but_contradictory_cas_turn_authority_is_an_invariant_failure() {
    ...
    let fixture = active_stop_fixture("stop-admission-wrong-cas-turn-index");
    let reverse = fixture
        .storage
        .cas_turn_owner(
            &fixture.store,
...
fn a_missing_ready_route_source_is_an_invariant_failure() {
    let fixture = active_stop_fixture("stop-admission-missing-ready-source");
...
fn a_missing_stopped_next_route_source_is_an_invariant_failure() {
    let fixture = active_stop_fixture("stop-admission-missing-next-source");
...
fn a_stopping_gate_without_its_selected_record_is_an_invariant_failure() {
    let fixture = active_stop_fixture("stop-admission-missing-stop");
    fixture.admit_stop();
    delete_fixture(
        &fixture.store,
        &fixture.storage,
