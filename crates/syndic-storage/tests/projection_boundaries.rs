...
#[path = "projection_boundaries/fixture.rs"]
mod fixture;
...
use syndic_storage::{
    ...
    ComposerPayload, CreateThread, DraftEditHistoryPolicyV1, ImageLabelOrdinal,
    ItemProjectionGeneration, MARKDOWN_CODE_INLINE_MAX_BYTES, MARKDOWN_CODE_INLINE_MAX_LINES,
...
use fixture::*;
use support::{TestHome, draft_id, exact_cas::submit_prepared_current_draft, id, open, timestamp};
...
fn paragraph_threshold_changes_one_inline_block_into_bounded_spans() {
    ...
    let exact = project_user_markdown("paragraph-exact", &exact_text);
    ...
    let over = project_user_markdown("paragraph-over", &over_text);
...
fn projection_pages_clamp_oversized_caller_limits() {
    ...
    let fixture = project_user_markdown("projection-page-clamp", &text);
    let page = fixture
        .storage
        .item_projections(
            &fixture.store,
...
fn heavy_resource_text_is_range_read_with_exact_bounded_continuations() {
    ...
    let fixture = project_user_markdown("resource-range", &markdown);
...
fn fenced_code_thresholds_crlf_and_unfinished_blocks_preserve_exact_source() {
    ...
    let crlf = "```rust\r\nlet value = 1;\r\n```\r\n";
    assert_inline_block("code-crlf", crlf, MarkdownBlockKind::FencedCode);
    let unfinished = "```rust\nlet value = 1;\n";
    assert_inline_block("code-unfinished", unfinished, MarkdownBlockKind::FencedCode);
}
...
fn table_thresholds_and_malformed_delimiters_preserve_exact_source() {
    ...
    assert_inline_block("table-exact-rows", &exact_rows, MarkdownBlockKind::Table);
    let over_rows = format!(
        "a|b\n---|---\n{}",
        "x|y\n".repeat(MARKDOWN_TABLE_INLINE_MAX_BODY_ROWS as usize + 1)
    );
    assert_resource_block("table-over-rows", &over_rows, MarkdownBlockKind::Table);
    ...
    assert_inline_block("table-exact-bytes", &exact_bytes, MarkdownBlockKind::Table);
    ...
    assert_resource_block("table-over-bytes", &over_bytes, MarkdownBlockKind::Table);
