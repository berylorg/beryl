...
fn prepared_and_ready_flight_drops_release_the_bounded_runtime_slot() {
    ...
    let (_home, store, storage, thread) = fixture_with_faults("attempt-bound", 90, faults);
...
fn exact_pairing_one_shot_and_final_eof_initialization() {
    let faults = FaultController::new();
    let (_home, store, storage, thread) = fixture_with_faults("linear", 1, faults.clone());
...
fn ready_flight_is_rejected_after_its_home_generation_retires() {
    ...
    let (_home, store, storage, thread) =
        fixture_with_faults("ready-generation", 55, faults.clone());
...
fn indeterminate_exact_new_reopens_same_generation_and_finishes_the_page() {
    let faults = FaultController::new();
    let (_home, store, storage, thread) = fixture_with_faults("indeterminate", 60, faults.clone());
...
fn reconciliation_error_preserves_exact_pending_owner_for_retry() {
    let faults = FaultController::new();
    let (_home, store, storage, thread) =
        fixture_with_faults("reconciliation-retry", 120, faults.clone());
    let (_foreign_home, foreign_store, _foreign_storage, _foreign_thread) =
        fixture_with_faults("reconciliation-retry-foreign", 121, FaultController::new());
...
fn lost_ack_reconciles_after_recovery_without_reviving_the_prior_generation() {
    let faults = FaultController::new();
    let (_home, store, storage, thread) = fixture_with_faults("recovered-ack", 110, faults.clone());
...
fn collision_is_operation_scoped_and_refusal_releases_runtime_capacity() {
    let faults = FaultController::new();
    let (_home, store, storage, thread) = fixture_with_faults("collision", 80, faults.clone());
