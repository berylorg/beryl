...
fn admission_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        ...
        let fixture =
            active_stop_fixture_with_faults(&format!("stop-admit-{name}"), faults.clone());
...
fn cause_join_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        let faults = FaultController::new();
        let fixture = active_stop_fixture_with_faults(&format!("stop-join-{name}"), faults.clone());
        fixture.admit_stop();
...
fn dispatch_claim_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        let faults = FaultController::new();
        let fixture =
            active_stop_fixture_with_faults(&format!("stop-claim-{name}"), faults.clone());
        fixture.admit_stop();
...
fn safe_reopen_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        let faults = FaultController::new();
        let fixture =
            active_stop_fixture_with_faults(&format!("stop-reopen-{name}"), faults.clone());
        fixture.admit_stop();
...
fn abandonment_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        let faults = FaultController::new();
        let fixture =
            active_stop_fixture_with_faults(&format!("stop-abandon-{name}"), faults.clone());
        fixture.admit_stop();
...
fn matching_terminal_fault_cuts_reconcile_to_one_whole_state() {
    for (name, point, expected) in cuts() {
        let faults = FaultController::new();
        let fixture =
            active_stop_fixture_with_faults(&format!("stop-terminal-{name}"), faults.clone());
        fixture.admit_stop();
        correlate_user_item(
            &fixture.store,
            &fixture.storage,
