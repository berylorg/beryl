...
fn admission_reconciles_and_retains_the_exact_stop_cut() {
    let fixture = active_stop_fixture("stop-admission");
    assert_eq!(
        fixture
            .storage
...
fn multiple_admission_causes_share_first_and_reconcile_as_the_exact_initial_set() {
    let fixture = active_stop_fixture("stop-multiple-admission-causes");
    let initial =
...
fn an_admission_cause_is_not_an_exact_later_join_after_an_unrelated_join() {
    let fixture = active_stop_fixture("stop-admission-cause-not-later-join");
    fixture.admit_stop();
...
fn target_disagreement_is_a_reconciliation_collision() {
    let fixture = active_stop_fixture("stop-target-collision");
    fixture.admit_stop();
...
fn pending_published_target_stops_and_consumes_a_matching_terminal_without_activation() {
    let fixture = pending_stop_fixture("stop-pending-terminal");
    let before = fixture
        .storage
...
fn cause_claim_and_safe_reopen_are_exact_monotonic_transitions() {
    let fixture = active_stop_fixture("stop-live-transitions");
    fixture.admit_stop();
    let gate_revision = fixture.gate().revision();
...
fn join_and_claim_orders_retain_distinct_exact_provenance_through_consumption() {
    let join_first = active_stop_fixture("stop-join-then-claim");
    ...
    let claim_first = active_stop_fixture("stop-claim-then-join");
...
fn claim_before_an_intervening_cause_join_does_not_authenticate_the_later_claim() {
    let fixture = active_stop_fixture("stop-stale-claim-after-join");
    fixture.admit_stop();
    let gate_revision = fixture.gate().revision();
...
fn interrupting_approval_prevents_safe_reopen() {
    let fixture = active_stop_fixture("stop-approval-reopen");
    fixture.admit_stop();
    let gate_revision = fixture.gate().revision();
    let join = JoinStopCause::new(
        fixture.operation_id,
...
fn matching_terminal_atomically_consumes_the_live_stop() {
    let fixture = active_stop_fixture("stop-matching-terminal");
    fixture.admit_stop();
    correlate_user_item(
        &fixture.store,
        &fixture.storage,
...
fn consuming_transitions_win_cleanly_against_a_stale_cause_join() {
    let reopened = active_stop_fixture("stop-join-races-reopen");
    reopened.admit_stop();
    ...
    let terminal = active_stop_fixture("stop-join-races-terminal");
    terminal.admit_stop();
