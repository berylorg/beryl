...
pub(super) fn establish_turn(
    ...
    let (execution, cas_thread, lineage, native_turn_count, profile) =
        match prior.as_ref().map(|record| record.state()) {
            ...
            _ => {
                ...
                (
                    execution,
                    CasThreadId::new(format!("stop-{turn}")).unwrap(),
    ...
    let gate = storage
        ...
        .unwrap()
        .unwrap();
    let cas_turn = CasTurnId::new(format!("stop-turn-{turn}")).unwrap();
    execute(
        store,
...
pub fn correlate_user_item(
    ...
    let cas_item = CasItemId::new(format!("stop-user-{item_id}")).unwrap();
