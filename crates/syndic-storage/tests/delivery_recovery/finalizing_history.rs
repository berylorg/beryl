...
fn finalizing_history_reopens_classifies_and_routes_new_input_as_terminal_history() {
    let recovery = terminal_home("finalizing-reopen", 501, false);
...
fn release_refuses_a_finalizable_item_until_item_and_transcript_converge() {
    let recovery = terminal_home("finalizing-item-refusal", 502, true);
...
fn stale_completion_proof_consumes_multiple_queued_admission_descendants() {
    let recovery = terminal_home("finalizing-compatible-descendants", 503, false);
    finish_transcript(&recovery.store, recovery.storage.clone(), recovery.thread);
...
fn successful_completed_history_stays_complete_across_queued_admission_and_release() {
    let recovery = terminal_home("successful-history-preserved", 505, true);
    converge_first_item(
        &recovery.store,
        recovery.storage.clone(),
        recovery.thread,
...
fn completion_rejects_an_impossible_finalizing_gate_descendant() {
    let recovery = terminal_home("impossible-finalizing-descendant", 506, false);
    finish_transcript(&recovery.store, recovery.storage.clone(), recovery.thread);
    let gate = recovery
        .storage
        .input_gate(&recovery.store, recovery.thread, point_limit())
...
fn gate_compatibility_rejects_a_skipped_route_generation() {
    let recovery = terminal_home("skipped-route-generation", 507, false);
