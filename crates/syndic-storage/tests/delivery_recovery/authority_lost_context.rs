...
fn context_fixture(name: &str, marker: bool) -> ContextFixture {
    ...
    crate::recovery_support::execute(
        ...
        storage.clone().abandon_active_binding(
            ...
            active
                .generic_abandonment(" authority-lost context", observed_at)
                .unwrap(),
        ),
    );
    let state = storage
...
fn exact_source_less_authority_lost_parent_replays_after_reopen_as_context() {
    let fixture = context_fixture("authority-lost-context-reopen", false);
    install_incomplete(
        &fixture,
        TurnIncompleteReason::AuthorityLost,
        None,
...
fn sourceful_or_differently_incomplete_parent_is_rejected() {
    let sourceful = context_fixture("authority-lost-sourceful", false);
    install_incomplete(
        &sourceful,
        TurnIncompleteReason::AuthorityLost,
    ...
    .enumerate()
    {
        let fixture = context_fixture(&format!("authority-lost-other-reason-{index}"), false);
        install_incomplete(
            &fixture,
            reason,
            None,
...
fn missing_or_mismatched_authority_lost_terminal_is_rejected() {
    let mismatched = context_fixture("authority-lost-mismatched-terminal", false);
    ...
    let missing = context_fixture("authority-lost-missing-terminal", false);
...
fn incomplete_capture_frontiers_and_unsupported_items_remain_ineligible() {
    ...
    for (index, frontiers) in variants.into_iter().enumerate() {
        let fixture = context_fixture(&format!("authority-lost-frontier-{index}"), false);
        install_incomplete(
            &fixture,
            TurnIncompleteReason::AuthorityLost,
            None,
    ...
    let marker = context_fixture("authority-lost-marker", true);
    install_incomplete(
        &marker,
        TurnIncompleteReason::AuthorityLost,
        None,
