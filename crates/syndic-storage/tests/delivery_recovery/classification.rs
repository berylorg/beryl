...
fn safe_pending_accepts_unbound_valid_and_stale_non_active_bindings() {
    let unbound = pending_home("safe-pending-unbound", 401);
    assert!(matches!(
        unbound
            .storage.clone()
    ...
    let valid = pending_home("safe-pending-valid", 402);
    let active = activate(
        &valid.store,
        valid.storage.clone(),
        valid.thread,
...
fn active_classification_covers_pre_and_post_cas_turn_publication() {
    for (value, publish_cas_turn) in [(410, false), (411, true)] {
        let recovery = pending_home("active-classification", value);
        ...
        let request = active
            .generic_abandonment(" startup authority lost", active.minimum_timestamp())
            .unwrap();
...
fn active_abandonment_authenticates_same_turn_post_abandonment_recovery() {
    let recovery = pending_home("post-abandonment", 420);
    activate(
        &recovery.store,
        recovery.storage.clone(),
        recovery.thread,
    ...
    let request = active
        .generic_abandonment(" recovered authority was lost", timestamp(7))
        .unwrap();
    execute(
        &recovery.store,
...
fn compaction_defers_work_and_idle_successor_settles_old_source() {
    let compacting = pending_home("deferred-compaction", 430);
    replace_gate_state(
        &compacting.store,
    ...
    let settled = pending_home("settled-successor", 431);
...
fn non_idle_gate_successor_is_reported_as_source_drift() {
    let recovery = pending_home("source-drift", 440);
    let source = startup_source(&recovery.store, recovery.storage.clone());
    replace_gate_state(
        &recovery.store,
        recovery.storage.clone(),
...
fn stable_unsupported_turn_state_is_reported_as_corruption() {
    let recovery = pending_home("corrupt-turn-state", 450);
    let source = startup_source(&recovery.store, recovery.storage.clone());
    let state = recovery
        .storage
        .clone()
...
fn pending_gate_for_a_noncurrent_tail_is_never_recoverable() {
    let recovery = pending_home("corrupt-pending-tail", 460);
...
fn active_snapshot_must_match_the_current_history_path() {
    let recovery = pending_home("corrupt-active-path", 470);
    activate(
        &recovery.store,
        recovery.storage.clone(),
        recovery.thread,
