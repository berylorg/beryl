...
fn active_marker_command_writer_cuts_recover_one_atomic_root_triplet() {
    ...
    .enumerate()
    {
        ...
        let fixture = prepare_marker_writer_cut(cut, faults.clone());
        let (fixture, source_roots, committed_target) =
            execute_marker_writer_cut(fixture, faults, fault_point);
        recover_marker_writer_cut(fixture, source_roots, committed_target, fault_point);
    }
}

#[cfg(feature = "test-faults")]
struct MarkerWriterCutFixture {
    home: TestHome,
    store: HomeStore,
    ...
    fragments: Vec<syndic_storage::DraftPieceBuildFragmentV1>,
}

#[cfg(feature = "test-faults")]
fn prepare_marker_writer_cut(cut: usize, faults: FaultController) -> MarkerWriterCutFixture {
...
#[cfg(feature = "test-faults")]
fn execute_marker_writer_cut(
    fixture: MarkerWriterCutFixture,
    faults: FaultController,
    fault_point: FaultPoint,
) -> (
    MarkerWriterCutFixture,
    syndic_storage::DraftPieceBuildRootsV1,
    bool,
) {
    let MarkerWriterCutFixture {
        home: _home,
        store,
        storage,
        session,
    ...
    let source_roots = loop {
        ...
        let preceding = storage
            ...
            .unwrap()
            .unwrap();
        committed(execute(&store, storage.advance_draft_piece_edit(preceding)));
    };
    let advance = storage
        .prepare_draft_piece_build_advance(
            &store,
...
#[cfg(feature = "test-faults")]
fn recover_marker_writer_cut(
    fixture: MarkerWriterCutFixture,
    source_roots: syndic_storage::DraftPieceBuildRootsV1,
    ...
    if let Some(pending) = pending {
        ...
        let retry = storage
            ...
            .unwrap()
            .unwrap();
        committed(execute(&store, storage.advance_draft_piece_edit(retry)));
    }
        ...
        })
    {
        ...
        assert!(matches!(
            execute(&store, storage.advance_draft_piece_edit(replay),),
            CommandOutcome::NotCommitted {
