...
#[test]
fn exact_mixed_root_materializes_to_one_sealed_reusable_mapping() {
    let (_home, store, storage, thread) = fixture("mixed", 11);
    let marker_a = DraftPieceMarkerV1::new(
        SyndicDraftMarkerId::from_bytes([31; 16]),
        1,
        ImageLabelOrdinal::new(3).unwrap(),
    );
    let marker_b = DraftPieceMarkerV1::new(
    ...
    let root = replace_empty(
        ...
        21,
        vec![
            DraftPieceV1::Text("hello 💎".to_owned()),
            DraftPieceV1::Marker(marker_a),
            DraftPieceV1::Marker(marker_b),
            DraftPieceV1::Text("\nworld".to_owned()),
        ],
    );
    let key = DraftComposerBuildKeyV1::new(
        ...
        DraftComposerMaterializationOperationIdV1::from_bytes([41; 16]),
    );
    committed(execute(
        &store,
    ...
    let mapping = loop {
        let status = storage
            ...
            .unwrap();
        match &status {
            DraftComposerMaterializationStatusV1::Building(phase) => match phase {
                DraftComposerBuildPhaseV1::Planning => saw_planning = true,
                DraftComposerBuildPhaseV1::Writing => saw_writing = true,
                DraftComposerBuildPhaseV1::Draining { .. } => saw_draining = true,
                _ => {}
            },
            DraftComposerMaterializationStatusV1::Sealed(mapping) => break *mapping,
            status => panic!("unexpected materialization status: {status:?}"),
        }
        let prepared = storage
            .prepare_draft_composer_materialization_step(&store, key)
            .unwrap_or_else(|error| panic!("prepare failed from {status:?}: {error:?}"))
            .expect("open build has a next bounded step");
    ...
    assert!(saw_planning && saw_writing && saw_draining);
    assert_eq!(mapping.key().source(), root);
    assert_eq!(
        mapping.content().id().as_bytes(),
        &[
            141, 224, 141, 123, 176, 213, 221, 31, 186, 187, 140, 102, 241, 254, 37, 196
        ]
    );
    assert_eq!(
        mapping.content().summary().digest().as_bytes(),
        &[
            26, 228, 141, 243, 80, 211, 27, 133, 112, 112, 111, 39, 89, 130, 221, 67, 127, 125,
            150, 58, 233, 97, 14, 213, 100, 165, 171, 147, 190, 86, 196, 203,
        ]
    );
    assert_eq!(
        mapping.content().summary().marker_digest(),
        [
            98, 34, 146, 9, 80, 166, 176, 98, 133, 219, 198, 117, 69, 38, 57, 172, 202, 220, 162,
            47, 226, 151, 131, 92, 158, 200, 226, 237, 44, 227, 232, 57,
        ]
    );
    assert_eq!(
        mapping.content().summary().atom_count(),
        root.summary().piece_count()
    );
    assert_eq!(
        mapping.content().summary().logical_utf8_bytes(),
        root.summary().logical_utf8_bytes()
    );
    assert_eq!(mapping.content().summary().image_marker_count(), 2);
    assert_eq!(
        mapping.content().summary().maximum_image_label(),
        Some(ImageLabelOrdinal::new(7).unwrap())
    );

    let mut expected = vec![1];
    expected.extend_from_slice(&4_u64.to_be_bytes());
    append_text_atom(&mut expected, "hello 💎");
    append_marker_atom(&mut expected, marker_a);
    append_marker_atom(&mut expected, marker_b);
    append_text_atom(&mut expected, "\nworld");
    let chunks = storage
        .content_chunks(
            &store,
            mapping.content().id(),
        ...
        .unwrap();
    assert_eq!(chunks.records().len(), 1);
    assert_eq!(chunks.records()[0].bytes(), expected);
    assert_eq!(candidate_head(storage, &store, thread).newest_root(), root);
    assert_eq!(
        storage
    ...
    assert_eq!(
        storage
            .draft_composer_materialization_status(&store, key)
            .unwrap(),
        DraftComposerMaterializationStatusV1::Sealed(mapping),
    );
    let other_operation = materialization_key(root, 42);
    assert!(matches!(
        execute(
            &store,
    ...
    assert_eq!(
        storage
            .draft_composer_materialization_status(&store, other_operation)
            .unwrap(),
        DraftComposerMaterializationStatusV1::Sealed(mapping),
    );
    let newer_root = append_text(storage, &store, thread, 22, " later");
    assert_ne!(newer_root, root);
    assert_eq!(
        storage
            .draft_composer_materialization_status(&store, key)
            .unwrap(),
...
#[test]
fn marker_only_long_same_anchor_run_has_fixed_ordered_oracle() {
    let (_home, store, storage, thread) = fixture("marker-only", 43);
    let markers: Vec<_> = (0_u16..300)
        .map(|ordinal| {
            let mut marker_id = [0_u8; 16];
            marker_id[..2].copy_from_slice(&ordinal.to_be_bytes());
            DraftPieceMarkerV1::new(
                SyndicDraftMarkerId::from_bytes(marker_id),
                ordinal as u64 + 1,
                ImageLabelOrdinal::new(ordinal as u64 + 1).unwrap(),
            )
        })
        .collect();
    replace_empty(
        ...
        markers[..150]
            .iter()
            .copied()
            .map(DraftPieceV1::Marker)
            .collect(),
    );
    let after_all = DraftCompositePositionV1::new(0, DraftCompositeGapWitnessV1::AfterAll);
    ...
    let mapping = materialize(storage, &store, materialization_key(root, 46));
    assert_eq!(mapping.content().summary().logical_utf8_bytes(), 0);
    assert_eq!(mapping.content().summary().image_marker_count(), 300);
    assert_eq!(mapping.content().summary().piece_count(), 300);
    let mut expected = vec![1];
    expected.extend_from_slice(&300_u64.to_be_bytes());
...
#[cfg(feature = "test-faults")]
#[test]
fn build_ceiling_has_fixed_independent_worst_case_proof() {
    let (_home, store, storage, thread) = fixture("build-ceiling", 180);
    replace_empty(
        storage,
        &store,
        thread,
    ...
    append_text(storage, &store, thread, 182, &"b".repeat(32_460));
    append_text(storage, &store, thread, 183, &"c".repeat(32_000));
    append_text(storage, &store, thread, 184, &"d".repeat(32_522));
    let marker = DraftPieceMarkerV1::new(
        SyndicDraftMarkerId::from_bytes([185; 16]),
        1,
    ...
    assert_eq!(root.summary().logical_utf8_bytes(), 128_982);
    assert_eq!(root.summary().piece_count(), 5);
    assert_eq!(root.summary().marker_count(), 1);
    assert!(root.root_node().is_some());
    assert!(root.marker_index_root().is_some());
    let key = materialization_key(root, 186);
    committed(execute(
        &store,
        storage.begin_draft_composer_materialization(storage.revision(&store).unwrap(), key),
    ));
    let successor = DraftComposerMaterializationOperationIdV1::from_bytes([187; 16]);
    for _ in 0..4096 {
        if let Some(remaining) = draft_composer_full_carry_remaining_bytes(&store, storage, key) {
            ...
            assert_eq!(
                draft_composer_terminal_build_encoded_size(&store, storage, key, successor),
                Some(65_273),
            );
            assert!(draft_composer_terminal_build_has_maximal_shape(
                &store, storage, key, successor,
            ));
            assert!(65_273 <= 65_536);
            return;
        }
        let prepared = storage
            .prepare_draft_composer_materialization_step(&store, key)
    ...
    panic!("full second carry Writing frontier was not reached")
}
...
#[cfg(feature = "test-faults")]
#[test]
fn sealed_targeted_reads_reject_nonfrontier_payload_corruption() {
    for (index, corruption) in [
        DraftComposerOutputCorruption::Chunk,
        DraftComposerOutputCorruption::ByteSpan,
        DraftComposerOutputCorruption::TextSpan,
    ...
    .enumerate()
    {
        let (_home, store, storage, thread) =
            fixture("sealed-target-corruption", 180 + index as u8);
        let marker = DraftPieceMarkerV1::new(
            SyndicDraftMarkerId::from_bytes([190 + index as u8; 16]),
            1,
        ...
        replace_empty(
            ...
            thread,
            200 + index as u8,
            vec![DraftPieceV1::Text("x".repeat(60_000))],
        );
        append_text(
            storage,
            &store,
        ...
        let key = materialization_key(root, 210 + index as u8);
        let mapping = materialize(storage, &store, key);
        assert!(mapping.content().summary().chunk_count() >= 3);
        assert!(mapping.content().summary().piece_count() >= 4);
        let leading = storage
            .prove_sealed_content_text_segment(&store, mapping.content(), None)
            .unwrap()
            .unwrap();
        let after_marker = leading.following_marker().unwrap();
        committed(execute(
            &store,
        ...
        let targeted = match corruption {
            DraftComposerOutputCorruption::Chunk => storage
                .sealed_content_text_range(&store, mapping.content(), 0, 65_536)
                .map(|_| ()),
            DraftComposerOutputCorruption::ByteSpan | DraftComposerOutputCorruption::Piece => {
                storage
                    .prove_sealed_content_text_segment(&store, mapping.content(), None)
                    .map(|_| ())
            }
            DraftComposerOutputCorruption::TextSpan => storage
                .prove_sealed_content_text_segment(&store, mapping.content(), Some(after_marker))
                .map(|_| ()),
        };
        assert!(targeted.is_err());
    }
}
