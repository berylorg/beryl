...
fn activate_pending(
    ...
    let cas_thread = CasThreadId::new(format!("lifecycle-cas-{thread_byte}")).unwrap();
    ...
    let cas_turn = beryl_model::CasTurnId::new(format!("lifecycle-turn-{thread_byte}")).unwrap();
    if publish_cas_turn {
        execute(
            store,
...
fn queued_input_survives_active_abandonment_retry_and_activation() {
    ...
    let retry = valid_request(
        ...
        fixture.thread,
        fixture.selected,
        CasThreadId::new("abandonment-retry-cas").unwrap(),
    );
    execute(
        &store,
