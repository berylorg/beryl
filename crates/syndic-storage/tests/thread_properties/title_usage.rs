    ...
    fn new(name: &str) -> Self {
        loop {
            ...
            let path = std::env::temp_dir()
                .join(format!("beryl-{name}-{}-{sequence}", std::process::id()));
...
fn usage_survives_binding_advance_and_reopen_while_wrong_routes_are_rejected() {
    ...
    let cas_thread = CasThreadId::new("usage-route").unwrap();
    execute(
        &store,
    ...
    let wrong = usage_observation(valid_revision, CasThreadId::new("wrong-route").unwrap(), 1);
    execute_rejected(
        &store,
    ...
    execute_rejected(
        ...
        storage.publish_thread_usage(
            ...
            PublishThreadUsage::new(
                ...
                usage_observation(valid_revision, CasThreadId::new("usage-route").unwrap(), 1),
            ),
        ),
    );
    execute_rejected(
        ...
        storage.publish_thread_usage(
            ...
            PublishThreadUsage::new(
                ...
                usage_observation(
                    ...
                    CasThreadId::new("usage-route").unwrap(),
                    2,
                ),
            ),
        ),
    ...
    execute_rejected(
        ...
        storage.publish_thread_usage(
            ...
            PublishThreadUsage::new(
                ...
                usage_observation(valid_revision, CasThreadId::new("usage-route").unwrap(), 2),
            ),
        ),
    );
    execute(
