...
fn terminal_history_release_preserves_prior_retry_witness_across_reopen() {
    ...
    store
        .execute_current(storage.current_begin_accepted_input_delivery(begin))
        .unwrap();
    ...
    store
        .execute_current(storage.current_retry_accepted_input_delivery(retry.clone()))
        .unwrap();
    assert_eq!(
        storage
            .retry_accepted_input_delivery_status(&store, &retry, super::limit())
    ...
    .unwrap();
    store
        .execute_current(storage.current_admit_live_source_event(terminal))
        .unwrap();
