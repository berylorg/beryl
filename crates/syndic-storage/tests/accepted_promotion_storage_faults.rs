...
#[path = "accepted_promotion_storage_faults/corruption.rs"]
mod corruption;
#[path = "accepted_promotion_storage_faults/deep_tail.rs"]
mod deep_tail;
#[path = "accepted_promotion_storage_faults/descendant_collisions.rs"]
mod descendant_collisions;
#[path = "accepted_promotion_storage_faults/identity.rs"]
mod identity;
...
fn same_domain_revision_still_fences_every_exact_promotion_authority() {
    ...
    let (_source_home, source_store, source_storage) =
        seed("promotion-authority-source", fixture.records.clone());
    source_store
        .scrub_whole_home(beryl_home_store::WholeHomeScrubTrigger::Explicit)
        .unwrap();
    ...
    for drift in AuthorityDrift::ALL {
        ...
        let name = format!("promotion-authority-{}", drift.name());
