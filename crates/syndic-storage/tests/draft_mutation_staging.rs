...
fn one_page_payload_is_durable_before_bounded_builder_construction() {
    ...
    let head = storage
        ...
        .unwrap()
        .unwrap();
    let prepared = prepare_one_page_batch(
        &storage,
        &head,
        &session,
...
fn lanes_advance_independently_and_commands_reconcile_exactly() {
    ...
    let head = storage
        ...
        .unwrap()
        .unwrap();
    let source = prepare_one_page_batch(
        &storage,
        &head,
        &session,
...
fn occupied_next_fork_cursor_totals_and_overflow_fail_closed() {
    ...
    let accepted = prepare_one_page_batch(
        &storage,
        &source_head,
        &session,
        DraftMutationStagingLaneV1::Proposal,
    ...
    .unwrap();
    let collision = prepare_one_page_batch(
        &storage,
        &source_head,
        &session,
        DraftMutationStagingLaneV1::Proposal,
...
fn every_staging_command_class_reconciles_after_an_indeterminate_commit() {
    ...
    let head = storage
        ...
        .unwrap()
        .unwrap();
    let page = prepare_one_page_batch(
        &storage,
        &head,
        &session,
        DraftMutationStagingLaneV1::Proposal,
