...
fn marker_free_composer_pages_directly_into_one_utf8_string() {
    ...
    let fixture = seed("content-text-pages", &composer(&source));
...
fn pages_trim_at_utf8_boundaries_and_reject_nonprogress() {
    let fixture = seed("content-text-utf8", &composer("a💎b"));
    let first = fixture
        .storage
...
fn page_byte_counts_include_span_and_chunk_cursors() {
    let fixture = seed("content-text-accounting", &composer("accounted"));
    let spans = fixture
        .storage
        .content_text_spans(
            &fixture.store,
...
fn empty_content_and_exact_end_return_one_terminal_empty_page() {
    ...
    let fixture = seed("content-text-empty", &prepared);
    let page = fixture
        .storage
    ...
    let fixture = seed("content-text-exact-end", &composer("done"));
    let page = fixture
        .storage
...
fn text_only_boundary_rejects_marker_bearing_content() {
    ...
    let fixture = seed("content-text-marker", &prepared);
    assert!(matches!(
        fixture
            .storage
            .sealed_content_text_range(&fixture.store, fixture.content, 0, 64),
...
fn nonsealed_lifecycle_and_missing_chunk_reject_without_fallback() {
    ...
    let fixture = seed("content-text-lifecycle", &prepared);
    ...
    let fixture = seed("content-text-missing-chunk", &prepared);
...
fn payload_limit_and_out_of_range_offset_are_typed() {
    let fixture = seed("content-text-input-bounds", &composer("text"));
    assert!(matches!(
        fixture
            .storage
