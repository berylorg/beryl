...
fn exact_item_budget_reaches_index_scan_without_a_proportional_frontier() {
    ...
    let exact = build_budget_fixture("recovery-exact-item-budget", RecoveryItemCount::MAX - 1);
    ...
    let overflow = build_budget_fixture("recovery-item-budget-overflow", RecoveryItemCount::MAX);
    reset_recovery_residency_metrics();
    let error = overflow
        .storage
        .prepare_recovery_projection(
