...
#[test]
fn selected_turn_title_candidate_uses_selected_turn_prompt() {
    let candidate = ThreadTitleCandidate::for_selected_user_turn(
        "target_thread",
        "please improve the clicked turn title",
    )
    .unwrap();
    let mut backend = FakeTitleBackend::with_turn(completed_title_turn("Clicked Turn Title"))
        .with_expected_prompt_tag("<selected_user_turn>");
    ...
    assert_eq!(
        result,
        ThreadTitleResult::Applied {
            title: "Clicked Turn Title".to_string()
        }
    );
}
...
impl FakeTitleBackend {
    ...
    fn with_expected_prompt_tag(mut self, expected_prompt_tag: &'static str) -> Self {
        self.expected_prompt_tag = expected_prompt_tag;
        self
    }
}
...
impl ThreadTitleBackend for FakeTitleBackend {
    ...
    fn start_turn_with_options(
        ...
        assert!(text.contains(self.expected_prompt_tag));
