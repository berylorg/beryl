...
mod window_close;
#[path = "context_compaction/work_facts.rs"]
mod work_facts;
...
fn startup_consumes_provider_stop_without_ordinary_replay_and_preserves_accepted_next() {
    ...
    let submitted = source.submit_text(" provider-stop base turn");
    source.complete_with_assistant(submitted, " provider-stop answer");
    ...
    publish_provider_event(
        ...
        &source.storage,
        operation_id,
        CompactionProviderEvent::TurnStarted(CasTurnId::new("provider-stop").unwrap()),
        72_102,
    );
    ...
    source.accept_text(" accepted during provider stop");
