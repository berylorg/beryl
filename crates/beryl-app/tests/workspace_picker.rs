...
fn picker_current_workspace_row_uses_active_marker_without_current_label() {
    ...
    assert!(marker_body.contains("shell.role_color("));
    assert!(marker_body.contains("BerylThemeRole::WorkspacePickerRowActive"));
    assert!(!marker_body.contains("shell.role_foreground("));
    assert!(!marker_body.contains("shell.role_font_weight("));
...
#[test]
fn workspace_rename_rebinds_graph_settings_target_and_waits_for_graph_policy_save() {
    let shell_source = include_str!("../src/shell.rs");
    let blockers_body = rust_function_body(
        shell_source,
        "fn workspace_rename_blockers(&self) -> WorkspaceRenameBlockers {\n        let mut blockers",
    );
    let finish_body = rust_function_body(shell_source, "fn finish_workspace_title_update");

    assert!(blockers_body.contains("has_pending_graph_upkeep_save()"));

    let replace_index = finish_body
        .find("replace_manifest_for_rename")
        .expect("rename completion should replace the loaded manifest");
    let rebind_index = finish_body
        .find("rebind_graph_workspace_target_after_rename")
        .expect("rename completion should rebind graph settings target");
    let sync_index = finish_body
        .find("sync_settings_window_model")
        .expect("rename completion should sync settings window model");

    assert!(replace_index < rebind_index);
    assert!(rebind_index < sync_index);
}

#[test]
fn generated_workspace_title_waits_for_graph_policy_save() {
    let shell_source = include_str!("../src/shell.rs");
    let begin_body = rust_function_body(
        shell_source,
        "fn begin_workspace_title_generation_if_needed",
    );
    let poll_settings_body = rust_function_body(shell_source, "fn poll_settings_save");

    assert!(begin_body.contains("has_pending_graph_upkeep_save()"));
    assert!(begin_body.contains("blockers.persistence_work = graph_upkeep_save_pending"));
    assert!(!begin_body.contains("blockers.persistence_work = false"));
    assert!(begin_body.contains("self.pending_workspace_title_candidate = Some(candidate);"));
    assert!(poll_settings_body.contains("self.begin_workspace_title_generation_if_needed();"));
}
