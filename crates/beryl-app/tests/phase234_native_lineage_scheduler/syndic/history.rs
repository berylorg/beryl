...
impl Fixture {
    pub fn activate_without_terminal(&mut self, submitted: SubmittedTurn) -> CasTurnSource {
        self.activate_without_terminal_on(self.thread, submitted)
    }

    pub fn activate_without_terminal_on(
        &mut self,
    ...
    pub fn complete_active_without_assistant(
        ...
        self.complete_active_without_assistant_on(self.thread, submitted, source);
    }

    pub fn complete_active_without_assistant_on(
        &mut self,
        thread: SyndicThreadId,
        submitted: SubmittedTurn,
        source: &CasTurnSource,
    ...
    pub fn mark_active_unknown_terminal(
        ...
        self.mark_active_unknown_terminal_on(self.thread, submitted, source);
    }

    pub fn mark_active_unknown_terminal_on(
        &mut self,
        thread: SyndicThreadId,
        submitted: SubmittedTurn,
        source: &CasTurnSource,
    ...
    fn finalize_all(&mut self, thread: SyndicThreadId, turn: SyndicTurnId) {
        ...
        for index in indexes {
            ...
            if matches!(
                ...
                let command_home = self.store.live_home_command().unwrap();
                project_item(command_home.home(), self.storage.clone(), index.item_id());
            }
            let state = {
                let command_home = self.store.live_home_command().unwrap();
                self.storage
