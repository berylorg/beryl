...
impl RunIdentity {
    ...
    pub fn new(run_id: u64) -> Self {
        Self {
            run_id,
            thread_id: BoundedCasId::new("cas-thread-", run_id),
            turn_id: BoundedCasId::new("cas-turn-", run_id),
            item_id: BoundedCasId::new("user-item-", run_id),
        }
    }
...
impl BoundedCasId {
    fn new(prefix: &str, run_id: u64) -> Self {
        ...
        fmt::Write::write_fmt(&mut value, format_args!("{prefix}{run_id}"))
            .expect(" CAS identity fits fixed storage");
        value
    }

    fn as_str(&self) -> &str {
        std::str::from_utf8(&self.bytes[..usize::from(self.len)])
            .expect(" CAS identities are ASCII")
    }
}
