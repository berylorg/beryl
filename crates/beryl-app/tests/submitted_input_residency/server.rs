...
pub const AUTHORIZATION: &str = "Bearer submitted-input-residency";
...
impl RawCasServer {
    ...
    pub fn wait_for_request(&self) -> RequestOutcome {
        let ServerEvent::Request(outcome) = self.receive() else {
            panic!(" server did not report its request outcome")
        };
        outcome
    }
    ...
    pub fn assert_request_pending(&self) {
        match self.events.recv_timeout(Duration::from_millis(25)) {
            ...
            Err(mpsc::RecvTimeoutError::Disconnected) => {
                panic!(" server disconnected while its request should remain pending")
            }
            Ok(event) => panic!(" server advanced unexpectedly while blocked: {event:?}"),
        }
    }
    ...
    fn send_control(&self, control: ServerControl) {
        self.controls
            .send(control)
            .unwrap_or_else(|error| panic!(" server did not accept {control:?}: {error}"));
    }
    ...
    fn receive(&self) -> ServerEvent {
        self.events
            ...
            .unwrap_or_else(|error| panic!("timed out waiting for  server: {error}"))
    }
}
...
fn expect_control(controls: &Receiver<ServerControl>, expected: ServerControl) {
    let actual = controls
        .recv_timeout(TIMEOUT)
        .unwrap_or_else(|error| panic!("timed out waiting for  control {expected:?}: {error}"));
