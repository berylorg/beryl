...
use super::command::execute_exact_contribution as execute;
...
pub fn establish_turn(
    ...
    let (cas_thread, lineage, native_turn_count) = exact_source.unwrap_or_else(|| {
        (
            CasThreadId::new(format!("source-{thread}")).unwrap(),
    ...
    let gate = storage
        ...
        .unwrap()
        .unwrap();
    let cas_turn = CasTurnId::new(format!("turn-{turn}")).unwrap();
    execute(
        store,
...
pub fn admit_item_frame(
    ...
    let mut build =
        ...
        .unwrap()
        {
            ...
            outcome => panic!("expected clean sealed provider-frame staging, got {outcome:?}"),
        };
    for _ in 0..4_096 {
...
pub fn correlate_user_item(
    ...
    let cas_item_id = CasItemId::new(format!("user-{item_id}")).unwrap();
