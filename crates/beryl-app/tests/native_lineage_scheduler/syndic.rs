...
mod assets;
#[path = "../support/syndic_command.rs"]
mod command;
...
use beryl_model::{
    CasItemId, ExecutionBinding, SealedAssetReferenceSetProof, SyndicAcceptedInputId,
...
impl Fixture {
    ...
    pub fn accept_text(&mut self, text: &str) -> SyndicAcceptedInputId {
        self.accept_text_on(self.thread, text)
    }

    pub fn accept_text_on(&mut self, thread: SyndicThreadId, text: &str) -> SyndicAcceptedInputId {
        let (kind, source_draft) = self.submit_text_via_composer(thread, text);
    ...
    pub fn retire_current_binding(&mut self, thread: SyndicThreadId) {
        ...
        let BindingState::Valid(usable) = current.binding().state() else {
            panic!("fixture retires only a valid native binding")
        };
    ...
    pub fn create_ordinary(&mut self, seed: u8) -> SyndicThreadId {
        self.create_ordinary_with_execution(seed, execution_binding())
    }

    pub fn create_ordinary_with_execution(
        &mut self,
        seed: u8,
