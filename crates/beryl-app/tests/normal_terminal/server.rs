...
pub const AUTHORIZATION: &str = "Bearer normal-terminal";
pub const CAS_THREAD_ID: &str = "cas-thread";
pub const CAS_TURN_ID: &str = "cas-turn";
pub const CAS_ITEM_ID: &str = "user-item";
pub const CAS_COMMAND_ITEM_ID: &str = "command-item";
pub const SUBMITTED_TEXT: &str = " ordinary terminal success";
...
fn complete_steering_correlation_loss(
    ...
    send_json(
        socket,
        &format!(
            r#"{{"method":"item/started","params":{{"item":{{"type":"userMessage","id":"steering-item","clientId":"{correlation}","content":[{{"type":"text","text":"source-disagreeing steering input","text_elements":[]}}]}},"threadId":"{cas_thread_id}","turnId":"{CAS_TURN_ID}","startedAtMs":52002}}}}"#,
        ),
    );
...
fn send_command_execution(
    ...
    send_json(
        socket,
        &format!(
            r#"{{"method":"{method}","params":{{"item":{{"type":"commandExecution","id":"{CAS_COMMAND_ITEM_ID}","command":" command","cwd":"C:\\work\\beryl","status":"{status}","commandActions":[]}},"threadId":"{cas_thread_id}","turnId":"{CAS_TURN_ID}","{timestamp_field}":{timestamp}}}}}"#,
        ),
    );
}
