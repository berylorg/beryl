...
fn ack_loss_is_exactly_reconstructed_after_process_exit_and_reopen() {
    ...
    let output = std::process::Command::new(std::env::current_exe().expect("current test binary"))
        .arg("--exact")
        .arg("ack_loss_subprocess_child")
        .arg("--nocapture")
        .env("BERYL_CRASH_PARENT", parent.path())
        .output()
...
#[test]
fn ack_loss_subprocess_child() {
    let Some(parent) = std::env::var_os("BERYL_CRASH_PARENT") else {
        return;
    };
    let directory = tempfile::tempdir_in(parent).expect("child temp home");
    let fixture = Fixture::from_directory(directory, 120);
    println!("HOME={}", fixture._directory.path().display());
...
fn a_live_durable_job_makes_the_pristine_thread_ineligible_without_substitution() {
    ...
    let admission = BranchHandoffJobAdmission::new(
        ...
        ResolutionRequestIdentity::new(
            CasThreadId::new("child").expect("CAS thread"),
            CasTurnId::new("turn").expect("CAS turn"),
            DynamicToolCallId::new("tool").expect("tool call"),
        ),
