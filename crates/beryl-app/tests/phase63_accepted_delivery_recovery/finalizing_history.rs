...
fn source_less_finalizing_history_restart_converges_without_replay_and_reopen_is_idempotent() {
    ...
    assert_recovered_predecessor(&service, storage, &fixture);

    let revision = storage.revision(&service).unwrap();
    let gate = storage
        .input_gate(&service, fixture.thread, point_limit())
        .unwrap()
        .unwrap();
    let state = storage
        .turn_state(&service, fixture.predecessor, point_limit())
        .unwrap()
        .unwrap();
    let head = storage
        .transcript_view_head(&service, fixture.thread, point_limit())
        .unwrap()
        .unwrap();
    ...
    assert_eq!(storage.revision(&service).unwrap(), revision);
    assert_eq!(
        storage
            .input_gate(&service, fixture.thread, point_limit())
            .unwrap()
...
fn queued_successor_waits_for_terminal_history_gate_then_executes_exactly_once() {
    ...
    assert_eq!(
        accepted_route_state(&service, storage, &successor),
    ...
    if before_projection.fatal() {
        let thread = storage
            .thread(&service, fixture.thread, point_limit())
            .unwrap()
            .unwrap();
    ...
    assert_eq!(
        storage
            .turn(&service, promoted_turn, point_limit())
            .unwrap()
            .unwrap()
            .parent()
            .turn(),
    ...
    wait_until(
        ...
        || {
            let state = storage
                .turn_state(&service, promoted_turn, point_limit())
                .ok()
                .flatten()?;
            let gate = storage
                .input_gate(&service, fixture.thread, point_limit())
