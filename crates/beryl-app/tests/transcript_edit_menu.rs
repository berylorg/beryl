...
fn edit_target_rejects_non_backend_or_incomplete_history_rows() {
    ...
    assert_eq!(
        ...
        Some(TranscriptEditDisabledReason::MissingThreadId)
    );

    harness.materialize_live_turn("thread_a", "turn_live");
    assert!(harness.target_at(0, false).is_none());
    assert_eq!(
        disabled_reason_at_with_tail(&harness, 0, false),
        Some(TranscriptEditDisabledReason::CurrentTailUnknown)
    );
}
...
fn edit_target_rejects_placeholders_operational_rows_and_missing_user_input() {
    ...
    assert_eq!(
        disabled_reason_at(&harness, 0),
        Some(TranscriptEditDisabledReason::MissingUserInput)
    );
}
...
#[test]
fn edit_menu_gate_returns_disabled_rows_with_internal_reasons() {
    let mut harness = EditHarness::new();
    harness.replace_history(
        "thread_a",
    ...
    assert_disabled(
        ...
        TranscriptEditDisabledReason::TranscriptSelectionActive,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::SourceThreadNotIdle,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::SelectedThreadMismatch,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::SelectedThreadCompactionActive,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::PendingThreadActivation,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::RollbackCapabilityUnavailable,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::PendingTurnInput,
    );
    assert_disabled(
        target.clone(),
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::PendingActiveTurnSteering,
    );
    assert_disabled(
        target,
        TranscriptEditMenuGate {
        ...
        TranscriptEditDisabledReason::ConflictingSelectedThreadWork,
    );
}
...
fn assert_disabled(
    target: shell::TranscriptEditTarget,
    gate: TranscriptEditMenuGate,
    expected_reason: TranscriptEditDisabledReason,
) {
    let entry = transcript_edit_menu_entry(TranscriptEditTargetResolution::Enabled(target), gate)
        .expect("edit row should remain visible as disabled");
    assert_eq!(entry.disabled_reason(), Some(expected_reason));
}

fn disabled_reason_at(harness: &EditHarness, index: usize) -> Option<TranscriptEditDisabledReason> {
    disabled_reason_at_with_tail(harness, index, true)
}

fn disabled_reason_at_with_tail(
    harness: &EditHarness,
    index: usize,
    current_tail_known: bool,
) -> Option<TranscriptEditDisabledReason> {
    match harness.target_resolution_at(index, current_tail_known)? {
