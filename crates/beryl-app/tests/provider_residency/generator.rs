...
pub(crate) fn probe_while_blocked(socket: &mut WebSocket<TcpStream>, message: &mut WireMessage) {
    write_frame(socket.get_mut(), 0x9, true, b"provider-residency-probe").unwrap();
    let mut scratch = [0_u8; FRAME_PAYLOAD_BYTES];
    let count = message.fill(&mut scratch);
    assert!(!message.complete());
    ...
    loop {
        match socket.read() {
            Ok(Message::Pong(payload)) => {
                assert_eq!(payload.as_ref(), b"provider-residency-probe");
                return;
            }
...
pub(crate) fn write_missing_text(socket: &mut WebSocket<TcpStream>, sequence: u64) {
    let value = format!(
        r#"{{"method":"item/started","params":{{"item":{{"type":"agentMessage","id":"item-{sequence}"}},"threadId":"{CAS_THREAD_ID}","turnId":"{CAS_TURN_ID}","startedAtMs":123}}}}"#,
    );
