...
pub const AUTHORIZATION: &str = "Bearer recovery-history-stress";
pub const TIMEOUT: Duration = Duration::from_secs(60);

const TARGET_THREAD_PREFIX: &str = "recovery-target-";
...
fn complete_injection(
    ...
    let RequestOutcome::Complete(observation) = outcome else {
        panic!(" success request was aborted: {outcome:?}")
    };
...
fn complete_admission(socket: &mut WebSocket<TcpStream>) {
    ...
    let request = read_json(socket).expect("release-admission config request");
...
fn await_close(socket: &mut WebSocket<TcpStream>) {
    loop {
        match socket.read() {
            ...
            Err(error) => panic!(" server close wait failed: {error}"),
        }
    }
}
