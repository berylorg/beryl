...
use syndic_storage::{
    DraftEditorCandidateSessionIdV1, DraftPieceMalformedRangeRequestV1, DraftPieceMarkerAtV1,
    DraftPieceMarkerDemandV1, DraftPieceMarkerDirectionV1, DraftPieceMarkerEdgeProofRequestV1,
    DraftPieceMarkerEdgeProofV1, DraftPieceMarkerScopeV1, DraftPieceOperationIdV1,
    DraftPieceRangeSourceErrorV1, DraftPieceTextDemandV1,
};

use support::{current, fixture, point, populate};

#[cfg(feature = "test-faults")]
use support::{committed, execute, run_transaction, transaction};

#[cfg(feature = "test-faults")]
use syndic_storage::{DraftPieceReplacementV1, DraftPieceV1};
...
fn activation_and_bounded_range_custody_are_exact() {
    ...
    let root = current.draft().piece_root();
    ...
    let current_value = run(
        ...
        ComposerHostRequestKind::Text {
            ...
            demand: DraftPieceTextDemandV1::Validate(1),
            max_bytes: 4,
        },
    )
    .unwrap();
    assert!(matches!(
        current_value,
        ComposerHostResponseValue::CurrentText(Some(value))
            if value.value().bytes() == "α".as_bytes()
    ));
    assert!(matches!(
        run(
            &mut host,
        ...
        Ok(ComposerHostResponseValue::CurrentMarkers(Some(value)))
            if value.value().markers() == [DraftPieceMarkerAtV1::new(3, left)]
    ));
    assert!(matches!(
        run(
        ...
        Ok(ComposerHostResponseValue::CurrentMarkerProof(Some(value)))
            if matches!(value.value(), Some(DraftPieceMarkerEdgeProofV1::First { .. }))
    ));
    assert!(matches!(
        run(
            &mut host,
...
fn disposed_absent_corrupt_and_selector_drift_are_typed_and_atomic() {
    ...
    let ComposerHostActivationOutcome::Activated { binding, .. } = host
        .activate(
            &store,
            activation(thread, 86, 87, Vec::new()),
            &CommandCancellation::new(),
        )
        .unwrap()
    else {
    ...
    .enumerate()
    {
        ...
        let ComposerHostActivationOutcome::Activated { binding, .. } = host
            .activate(
                &store,
                activation(thread, 95 + case as u8, 97 + case as u8, Vec::new()),
                &CommandCancellation::new(),
            )
            .unwrap()
        else {
    ...
    host.test_arm_activation_after_selector_fault(move |store, storage| {
        run_transaction(storage, store, &successor, 3);
    });
    assert!(matches!(
        host.activate(
            &store,
...
#[cfg(feature = "test-faults")]
#[test]
fn current_and_candidate_marker_reads_reject_drift() {
    for proof in [false, true] {
        let (_home, store, storage, thread) =
            fixture("current-marker-drift", 110 + u8::from(proof));
        let (left, _) = populate(storage, &store, thread, 112 + u8::from(proof));
        let before = current(storage, &store, thread);
        let successor = transaction(
            storage,
            &before,
        ...
        arm_draft_piece_current_read_fault(move |store, storage| {
            run_transaction(storage, store, &successor, 4);
        });
        ...
        assert!(matches!(
            run(&mut host, &store, binding, 1, current_kind),
            Err(ComposerHostError::Range(
        ...
        assert!(matches!(
            ...
            Err(ComposerHostError::Range(
                DraftPieceRangeSourceErrorV1::Disposed(_)
            ))
        ));
    }
}
