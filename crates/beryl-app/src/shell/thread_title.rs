...
pub(super) struct ThreadTitleCandidate {
    ...
    user_input: String,
    prompt_source: ThreadTitlePromptSource,
}
...
impl ThreadTitleCandidate {
    pub(super) fn new(
        ...
        Self::with_prompt_source(
            target_thread_id,
            user_input,
            ThreadTitlePromptSource::FirstUserMessage,
        )
    }

    #[allow(dead_code)]
    pub(super) fn for_selected_user_turn(
        target_thread_id: impl Into<String>,
        user_input: impl Into<String>,
    ) -> Option<Self> {
        Self::with_prompt_source(
            target_thread_id,
            user_input,
            ThreadTitlePromptSource::SelectedUserTurn,
        )
    }

    fn with_prompt_source(
        target_thread_id: impl Into<String>,
        user_input: impl Into<String>,
        prompt_source: ThreadTitlePromptSource,
    ) -> Option<Self> {
    ...
    pub(super) fn prompt_source(&self) -> ThreadTitlePromptSource {
        self.prompt_source
    }
}
...
fn build_title_prompt(candidate: &ThreadTitleCandidate) -> String {
    match candidate.prompt_source() {
        ThreadTitlePromptSource::FirstUserMessage => {
            build_first_user_message_title_prompt(candidate.user_input())
        }
        ThreadTitlePromptSource::SelectedUserTurn => {
            build_selected_user_turn_title_prompt(candidate.user_input())
        }
    }
}

fn build_first_user_message_title_prompt(user_input: &str) -> String {
...
fn build_selected_user_turn_title_prompt(user_input: &str) -> String {
    let user_input = truncate_for_prompt(user_input.trim());
    format!(
        "Create a short title for a Codex conversation using the selected user turn below as the strongest title signal.\n\
Return only the title.\n\n\
<selected_user_turn>\n{user_input}\n</selected_user_turn>"
    )
}
