...
pub struct WorkspaceGraphUpkeepPolicy {
...
#[derive(Deserialize)]
struct WorkspaceGraphUpkeepPolicyRecord {
    #[serde(default)]
    instructions: Option<serde_json::Value>,
    #[serde(default)]
    graph_upkeep_instructions: Option<serde_json::Value>,
}
...
impl WorkspaceActivityPanelMode {
    pub fn panel_visible(self, parent_turn_active: bool, context_compaction_active: bool) -> bool {
        let _ = self;
        parent_turn_active || context_compaction_active
    }
}
...
impl WorkspaceGraphUpkeepPolicy {
    pub fn with_instructions(instructions: Option<String>) -> Self {
        Self {
            instructions: normalize_graph_upkeep_instructions(instructions),
        }
    }
    ...
    fn normalized(self) -> Self {
        Self::with_instructions(self.instructions)
    }
}
...
impl<'de> Deserialize<'de> for WorkspaceGraphUpkeepPolicy {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        let record = WorkspaceGraphUpkeepPolicyRecord::deserialize(deserializer)?;
        let instructions = record
            .instructions
            .or(record.graph_upkeep_instructions)
            .and_then(|value| match value {
                serde_json::Value::String(value) => Some(value),
                _ => None,
            });
        Ok(Self::with_instructions(instructions))
    }
}
...
impl BerylWorkspacePersistence {
    ...
    pub fn load_workspace_graph_upkeep_policy(
        &self,
        workspace_id: &BerylWorkspaceId,
    ) -> Result<WorkspaceGraphUpkeepPolicy, WorkspacePersistenceError> {
        self.ensure_workspaces_root()?;
        let database_path = self.workspace_database_path(workspace_id);
        if !database_path.exists() {
            return Ok(WorkspaceGraphUpkeepPolicy::default());
        }

        match load_workspace_record_from_database::<WorkspaceGraphUpkeepPolicy>(
            &database_path,
            WORKSPACE_GRAPH_UPKEEP_POLICY_KEY,
            "workspace graph upkeep policy",
        ) {
            Ok(policy) => Ok(policy.unwrap_or_default().normalized()),
            Err(error @ WorkspacePersistenceError::ParseWorkspaceRecord { .. }) => {
                warn!(
                    workspace_id = workspace_id.as_str(),
                    error = %error,
                    "ignoring malformed workspace graph upkeep policy record"
                );
                Ok(WorkspaceGraphUpkeepPolicy::default())
            }
            Err(error) => Err(error),
        }
    }

    pub fn save_workspace_graph_upkeep_policy(
        &self,
        workspace_id: &BerylWorkspaceId,
        ...
        self.ensure_workspaces_root()?;
        let database_path = self.workspace_database_path(workspace_id);
        if self.load_workspace_manifest(workspace_id)?.is_none() {
...
pub fn normalize_graph_upkeep_instructions_text(value: &str) -> Option<String> {
...
fn normalize_graph_upkeep_instructions(value: Option<String>) -> Option<String> {
    value.and_then(|value| normalize_graph_upkeep_instructions_text(&value))
}
