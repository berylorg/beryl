...
use gpui::{
    App, Application, AsyncApp, Bounds, ClipboardItem, Context, Entity, Image, ImageFormat,
    KeyBinding, KeyDownEvent, KeyUpEvent, MouseDownEvent, MouseMoveEvent, MouseUpEvent,
    PathPromptOptions, Pixels, Point, PromptButton, PromptLevel, ScrollHandle, ScrollWheelEvent,
    WeakEntity, Window, WindowBounds, WindowOptions, prelude::*, px, size,
};
use gpui_scrollbar::{
...
use crate::threaded_decision_archive_core::child_thread_is_read_only_decision_branch;
use crate::threaded_decision_dynamic_tools::is_beryl_threaded_decision_dynamic_tool;
use crate::{
...
fn retain_normal_selector_threads_for_decisions(
    known_threads: &mut Vec<ThreadSummary>,
    threaded_decision_state: &ThreadedDecisionState,
) {
    let hidden_thread_ids =
        crate::threaded_decision_archive_core::normal_selector_hidden_decision_child_thread_ids(
            threaded_decision_state,
        );
    if hidden_thread_ids.is_empty() {
        return;
    }
    known_threads.retain(|thread| {
        !hidden_thread_ids
            .iter()
...
fn transcript_scroll_position_diagnostic() -> TranscriptScrollPositionDiagnostic {
...
fn selected_thread_state_diagnostic_label(surface: &ConversationSurfaceState) -> &'static str {
    ...
    match surface.selected_thread_status.as_ref() {
        ...
        Some(status @ ThreadStatus::Active { .. }) if status.waiting_on_user_input() => {
            "waiting_on_user_input"
        }
        Some(ThreadStatus::Active { .. }) => "active",
...
gpui::actions!(
    ...
    [
        ...
        OpenTranscriptContextMenu,
        EditResidentContextTarget,
        BranchResidentContextTarget,
        PreviewResidentTranscriptMedia,
        CopyResidentTranscriptMedia,
        SaveResidentTranscriptMedia,
...
mod threaded_decision_resolution;
...
use threaded_decision_resolution::{
    DecisionResolutionGraphTask, PendingDecisionHandoffTurn, QueuedDecisionResolutionJob,
};
    ...
    application.run(move |cx: &mut App| {
        ...
        cx.bind_keys([
            ...
            KeyBinding::new(
                "ctrl-c",
                CopyTranscriptSelection,
                Some(syndic_transcript::SYNDIC_TRANSCRIPT_KEY_CONTEXT),
            ),
        ]);
        ...
        let workspace_persistence_store = app_state
            .as_ref()
            .ok()
            .map(|state| state.home_dir.workspace_persistence());
        ...
        let mut settings_state = match (
            ...
            ) => settings::SettingsState::new_with_theme_repository_and_workspace_persistence(
                active_theme.clone(),
...
pub(super) struct ShellView {
    ...
    decision_archive_receiver: Option<DecisionArchiveTask>,
    pending_decision_handoff_turn: Option<PendingDecisionHandoffTurn>,
    pending_decision_handoff_navigation: Option<PendingDecisionHandoffNavigation>,
    ...
    transcript_panel: Entity<syndic_transcript::SyndicTranscriptPanel>,
...
struct DiscoveryOutcome {
    ...
    workspace_ui_state: WorkspaceUiState,
    threaded_decision_state: ThreadedDecisionState,
    graph_upkeep_policy: WorkspaceGraphUpkeepPolicy,
}
...
impl ConversationSurfaceState {
    ...
    fn close_transcript_branch_menu(&mut self) -> bool {
        false
    }

    fn transcript_branch_menu_open(&self) -> bool {
        false
    }
    ...
    fn queue_pending_turn_fragment(
        ...
        let _ = (turn_index, fragment_index);
        self.transcript_user_scrolled = false;
        self.notices.clear_all();
        true
    }
    ...
    fn invalidate_stream_turns(
        ...
        let _ = thread_id;
        let _ = turn_ids.into_iter().count();
    }
...
impl ShellView {
    ...
    fn begin_transcript_host_activation_for_optional_thread(
        &mut self,
        ...
        if let Some(thread_id) = thread_id {
            self.begin_transcript_host_activation_for_thread(thread_id, source, cx);
        } else {
            self.begin_unavailable_transcript_host_activation(source, cx);
        }
    }

    fn begin_unavailable_transcript_host_activation(
        &mut self,
        source: syndic_transcript::TranscriptActivationSource,
        cx: &mut Context<Self>,
    ) {
    ...
    fn transcript_activation_source_for_thread_navigation(
        source: ThreadNavigationActivationSource,
    ) -> syndic_transcript::TranscriptActivationSource {
        match source {
            ThreadNavigationActivationSource::ThreadSelector => {
                syndic_transcript::TranscriptActivationSource::ThreadSelector
            }
    ...
    fn transcript_activation_source_for_selected_thread(
        source: SelectedThreadActivationSource,
    ) -> syndic_transcript::TranscriptActivationSource {
        match source {
            SelectedThreadActivationSource::StartupRestore => {
                syndic_transcript::TranscriptActivationSource::StartupRestore
            }
            SelectedThreadActivationSource::BackendReopenRefresh => {
                syndic_transcript::TranscriptActivationSource::BackendReopen
            }
            SelectedThreadActivationSource::Explicit(source) => {
                Self::transcript_activation_source_for_thread_navigation(source)
            }
        }
    }

    fn transcript_view_id_for_thread(thread_id: &str) -> syndic_transcript::TranscriptViewId {
    ...
    fn new(
        ...
        let transcript_panel = cx.new(|cx| syndic_transcript::SyndicTranscriptPanel::new(cx));
        ...
        let mut view = Self {
            ...
            decision_archive_receiver: None,
            pending_decision_handoff_turn: None,
            pending_decision_handoff_navigation: None,
            pending_decision_branch_jobs: VecDeque::new(),
            pending_decision_resolution_jobs: VecDeque::new(),
            pending_decision_archive_jobs: VecDeque::new(),
    ...
    fn cancel_thread_title_workers(&mut self) -> bool {
        ...
        let mut updated = self.finish_thread_title_task_outcomes(outcomes, false);
        let outcomes =
            thread_title::cancel_all_thread_title_tasks(&mut self.thread_title_update_receivers);
        updated |= self.finish_thread_title_update_task_outcomes(outcomes);
        updated
    }
    ...
    fn cancel_thread_title_workers_for_thread(&mut self, thread_id: &str) -> bool {
        ...
        let mut updated = self.finish_thread_title_task_outcomes(outcomes, false);
        let outcomes = thread_title::cancel_thread_title_tasks_for_thread(
            &mut self.thread_title_update_receivers,
            thread_id,
        );
        updated |= self.finish_thread_title_update_task_outcomes(outcomes);
        updated
    ...
    fn begin_application_shutdown(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.pending_decision_handoff_turn = None;
        self.pending_decision_branch_jobs.clear();
        self.pending_decision_resolution_jobs.clear();
        ...
        thread::spawn(move || {
            ...
            .and_then(|startup| {
                ...
                let threaded_decision_state = workspace_persistence
                    .load_workspace_threaded_decision_state(startup.active_workspace().id())
                    .map_err(crate::StartupStateError::WorkspacePersistence)?;
                let graph_upkeep_policy = workspace_persistence
                    .load_workspace_graph_upkeep_policy(startup.active_workspace().id())
                    .map_err(crate::StartupStateError::WorkspacePersistence)?;
    ...
    fn has_frame_poll_work(&self) -> bool {
        self.discovery_receiver.is_some()
            ...
            || !self.pending_decision_branch_jobs.is_empty()
    ...
    fn workspace_rename_blockers(&self) -> WorkspaceRenameBlockers {
        let mut blockers = WorkspaceRenameBlockers {
            ...
            graph_work: self.graph_receiver.is_some()
                || self.graph_thread_start_receiver.is_some()
                || self.decision_branch_start_receiver.is_some()
                || self.decision_child_progress_receiver.is_some()
                || self.decision_resolution_graph_receiver.is_some()
                || self.decision_archive_receiver.is_some(),
            transcript_work: !self.pending_decision_branch_jobs.is_empty()
                || !self.pending_decision_resolution_jobs.is_empty()
                || !self.pending_decision_archive_jobs.is_empty()
                || self
                    .decision_resolution_parent_activation_record_id
    ...
    fn poll(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        updated |= self.poll_workspace_updates(window, cx);
        ...
        updated |= self.begin_next_ready_queued_decision_branch(window, cx);
            ...
            match update {
                ...
                DiscoveryUpdate::Finished(Ok(outcome)) => {
                    ...
                    let loaded = LoadedWorkspaceState::new(
                        ...
                        outcome.threaded_decision_state,
                        startup_warning,
                    );
    ...
    fn poll_workspace_updates(&mut self, window: &mut Window, cx: &mut Context<Self>) -> bool {
        ...
        loop {
            ...
            match update {
                ...
                WorkspaceUpdate::Finished(result) => {
                    self.workspace_receiver = None;
                    self.workspace_open_cancellation = None;
                    self.finish_workspace_open(result, cx);
                    updated = true;
                    break;
                }
            }
    ...
    fn poll_thread_activation_updates(
        ...
        match receiver.try_recv() {
            Ok(ThreadActivationUpdate::Finished(outcome)) => {
                self.thread_activation_receiver = None;
                let activation_finish = self.finish_thread_activation_worker(outcome, cx);
                updated = true;
        ...
        loop {
            ...
            let update = match next_update {
                ...
                Err(TryRecvError::Disconnected) => {
                    ...
                    self.pending_lifecycle_phase_continue = None;
                    self.cancel_pending_decision_handoff_turn_after_worker_stop();
            ...
            match update {
                ...
                TurnWorkerUpdate::ThreadTitleEligible {
                    ...
                    updated |= self.handle_thread_title_candidate_from_turn(
                        execution_target,
                        candidate,
                        title_mode,
                    );
                ...
                TurnWorkerUpdate::Event(event) => {
                    let refresh_inventory_for_event = matches!(
                        &event,
                        beryl_backend::TurnStreamEvent::ThreadArchived { .. }
                            | beryl_backend::TurnStreamEvent::ThreadUnarchived { .. }
                    );
                    ...
                    let suppress_bootstrap_title_candidate = false;
                    ...
                    if let Some((thread_id, turn_id)) = started_turn.as_ref() {
                        updated |= self.note_decision_child_turn_started(thread_id, turn_id, cx);
                    }
                    if let Some((thread_id, turn_id)) = started_turn.as_ref() {
                        updated |=
                            self.note_decision_handoff_turn_started(thread_id, turn_id, window, cx);
                    }
                    if let Some(surface) = self.conversation_surface_mut() {
                        ...
                        if suppress_bootstrap_title_candidate {
                            applied_stream_event.title_candidate = None;
                        }
                        if let Some((thread_id, turn_id)) = started_turn.as_ref()
                    ...
                    if refresh_inventory_for_event {
                        self.mark_member_thread_inventory_refresh_needed();
                        updated = true;
                    }
                ...
                TurnWorkerUpdate::Finished(outcome) => {
                    ...
                    if let Some(thread_id) = pending_thread_id {
                        ...
                        } else if self.has_queued_decision_resolution_jobs_for_child(&thread_id) {
        ...
        loop {
            ...
            match next_request {
                Ok(request) => {
                    ...
                    if is_beryl_threaded_decision_dynamic_tool(request.request()) {
                        let response = self.handle_beryl_threaded_decision_dynamic_tool_request(
                            request.request(),
                            window,
                            cx,
                        );
    ...
    fn handle_thread_title_candidate_from_turn(
        &mut self,
        execution_target: WorkspaceId,
        candidate: thread_title::ThreadTitleCandidate,
        title_mode: TurnThreadTitleMode,
        ...
        match title_mode {
            TurnThreadTitleMode::Disabled => false,
            TurnThreadTitleMode::AutomaticIfMissing => {
                self.repair_thread_title_from_candidate(execution_target, candidate)
            }
            TurnThreadTitleMode::BranchRetitleAfterFirstUserTurn => {
                self.begin_branch_thread_retitle_from_candidate(execution_target, candidate)
            }
        }
    }
    ...
    fn begin_branch_thread_retitle_from_candidate(
        ...
        let thread_id = candidate.target_thread_id().to_string();
        if !self.branch_thread_retitle_pending(&thread_id) {
            return false;
        }
        if self.thread_has_manual_gui_title(&thread_id) {
            return self.mark_branch_thread_title_retitle_finished(&thread_id);
        }
        if self.thread_title_task_active_for_thread(&thread_id)
            || !self.thread_title_worker_capacity_available()
        {
            return false;
        }
        if !self.begin_thread_title_update(execution_target, candidate) {
            return false;
        }

        self.mark_branch_thread_title_retitle_started(&thread_id)
    }

    fn begin_pending_branch_retitle_from_known_history(&mut self, thread_id: &str) -> bool {
        if !self.branch_thread_retitle_pending(thread_id)
            || self.thread_title_task_active_for_thread(thread_id)
        {
            return false;
        }
        if self.thread_has_manual_gui_title(thread_id) {
        ...
        let thread_id_model = ConversationThreadId::new(thread_id.to_string());
        let Some((execution_target, user_input)) =
            self.workspace_shell_state().and_then(|loaded| {
                let registration = loaded
                    .workspace_state
                    .thread_registration(&thread_id_model)?;
                let surface = self.conversation_surface()?;
                (surface.selected_thread_id() == Some(thread_id)).then(|| {
                    first_real_branch_user_input_fragment_text(surface, registration)
                        .map(|input| (registration.execution_target().clone(), input.to_string()))
                })?
            })
        else {
            return false;
        ...
        let Some(candidate) =
            thread_title::ThreadTitleCandidate::new(thread_id.to_string(), user_input)
        else {
            return false;
        };
    ...
    fn thread_has_manual_gui_title(&self, thread_id: &str) -> bool {
        self.workspace_shell_state()
            ...
            .and_then(|thread| thread.gui_title())
            .is_some_and(|title| title.source() == ConversationThreadTitleSource::Manual)
    }
    ...
    fn earliest_known_user_input_for_thread(&self, thread_id: &str) -> Option<String> {
        let surface = self.conversation_surface()?;
        if surface.selected_thread_id() != Some(thread_id) {
            return None;
        }

        if let Some(registration) = self.workspace_shell_state().and_then(|loaded| {
            loaded
                .workspace_state
                .thread_registration(&ConversationThreadId::new(thread_id.to_string()))
        }) && registration.branch_title_retitle_state()
            != BranchThreadTitleRetitleState::NotBranch
        {
            return first_real_branch_user_input_fragment_text(surface, registration)
                .map(str::to_string);
        }
    ...
    fn thread_title_task_active_for_thread(&self, thread_id: &str) -> bool {
        thread_title::thread_title_task_active_for_thread(&self.thread_title_receivers, thread_id)
            || thread_title::thread_title_task_active_for_thread(
                &self.thread_title_update_receivers,
                thread_id,
            )
    }

    fn thread_title_worker_count(&self) -> usize {
        self.thread_title_receivers.len() + self.thread_title_update_receivers.len()
    }
    ...
    fn begin_thread_title_generation(
        ...
        if !self.thread_title_worker_capacity_available() {
            warn!(
    ...
    fn begin_thread_title_update(
        ...
        if !self.thread_title_worker_capacity_available() {
            warn!(
                thread_id = candidate.target_thread_id(),
                max_workers = thread_title::MAX_THREAD_TITLE_WORKERS,
                "skipping user-initiated thread-title update because the worker limit is reached"
            );
            return false;
        }
        ...
        let Some(connector) = self.backend_client_connector_for_execution_target(&execution_target)
            ...
            warn!(
                thread_id = candidate.target_thread_id(),
                runtime_target = %execution_target.display_label(),
                "skipping user-initiated thread-title update because no backend connector is available for the target"
            );
            return false;
        };
    ...
    fn poll_thread_title_updates(&mut self) -> bool {
        ...
        let mut updated = self.finish_thread_title_task_outcomes(outcomes, true);
        let outcomes =
            thread_title::poll_thread_title_tasks(&mut self.thread_title_update_receivers);
        updated |= self.finish_thread_title_update_task_outcomes(outcomes);
        updated
    }
    ...
    pub(super) fn thread_title_mode_for_user_submission(
        &self,
        thread_id: Option<&str>,
        automatic_title_generation_allowed: bool,
    ) -> TurnThreadTitleMode {
        let Some(loaded) = self.workspace_shell_state() else {
            return TurnThreadTitleMode::automatic_if_allowed(automatic_title_generation_allowed);
        };
        Self::thread_title_mode_for_user_submission_state(
    ...
    fn thread_title_mode_for_user_submission_state(
        ...
        if let Some(thread_id) = thread_id {
            let thread_id = ConversationThreadId::new(thread_id.to_string());
            if workspace_state.thread_branch_title_retitle_pending(&thread_id) {
                return TurnThreadTitleMode::BranchRetitleAfterFirstUserTurn;
            }
        }
    ...
    fn finish_thread_title_update_task_outcomes(
        &mut self,
        outcomes: Vec<ThreadTitleTaskOutcome>,
    ) -> bool {
        let mut updated = false;
        for outcome in outcomes {
            match outcome {
                ThreadTitleTaskOutcome::Finished { thread_id, result } => {
                    updated |= self.finish_thread_title_update_request(thread_id, result);
                }
                ThreadTitleTaskOutcome::Abandoned { thread_id } => {
                    debug!(
                        thread_id = thread_id.as_str(),
                        "user-initiated thread-title update worker was cancelled"
                    );
                    updated = true;
                }
                ThreadTitleTaskOutcome::Disconnected { thread_id } => {
                    warn!(
                        thread_id = thread_id.as_str(),
                        "user-initiated thread-title update worker stopped before returning a result"
                    );
                    updated = true;
                }
            }
    ...
    fn finish_thread_title_update(&mut self, thread_id: String, result: ThreadTitleResult) -> bool {
        ...
        updated |= self.begin_pending_branch_retitle_from_known_history(&thread_id);
        updated
    }

    fn finish_thread_title_update_request(
        &mut self,
        thread_id: String,
        ...
        match result {
            ...
            ThreadTitleResult::Failed { message } => {
                warn!(
                    thread_id = thread_id.as_str(),
                    error = %message,
                    "user-initiated thread-title update failed"
                );
                let updated = self.mark_branch_thread_title_retitle_finished(&thread_id);
                updated
                    | self.block_if_backend_process_dead(
                        "Managed backend disconnected during title update",
                        "The backend process exited while Beryl was updating the thread title.",
                        &message,
                    )
            }
        }
    ...
    fn branch_thread_retitle_pending(&self, thread_id: &str) -> bool {
        let thread_id = ConversationThreadId::new(thread_id.to_string());
        self.workspace_shell_state().is_some_and(|loaded| {
            loaded
                .workspace_state
                .thread_branch_title_retitle_pending(&thread_id)
        })
    }

    fn mark_branch_thread_title_retitle_started(&mut self, thread_id: &str) -> bool {
        let thread_id = ConversationThreadId::new(thread_id.to_string());
        let changed = {
            let Some(loaded) = self.workspace_shell_state_mut() else {
                return false;
                ...
                .mark_thread_branch_title_retitle_started(&thread_id)
            {
                ...
                Err(error) => {
                    warn!(
                        thread_id = thread_id.as_str(),
                        error = %error,
                        "could not record branch thread-title retitle start"
                    );
                    false
                }
            }
    ...
    fn mark_branch_thread_title_retitle_finished(&mut self, thread_id: &str) -> bool {
        let thread_id = ConversationThreadId::new(thread_id.to_string());
        let changed = {
            let Some(loaded) = self.workspace_shell_state_mut() else {
                return false;
                ...
                .mark_thread_branch_title_retitle_finished(&thread_id)
            {
                ...
                Err(error) => {
                    warn!(
                        thread_id = thread_id.as_str(),
                        error = %error,
                        "could not record branch thread-title retitle completion"
                    );
                    false
                }
            }
    ...
    fn begin_workspace_title_generation_if_needed(&mut self) -> bool {
        ...
        let graph_upkeep_save_pending = self.settings_state.has_pending_graph_upkeep_save();
        let mut blockers = self.workspace_rename_blockers();
        blockers.persistence_work = graph_upkeep_save_pending;
        if blockers.any() {
            let candidate_workspace_id = candidate.workspace_id().clone();
            if graph_upkeep_save_pending {
                self.pending_workspace_title_candidate = Some(candidate);
            }
            warn!(
                workspace_id = candidate_workspace_id.as_str(),
    ...
    fn poll_workspace_title_updates(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> bool {
        ...
        match receiver.try_recv() {
            Ok(WorkspaceTitleUpdate::Generated {
                workspace_id,
                result,
            }) => self.finish_workspace_title_update(workspace_id, result, false, window, cx),
            Ok(WorkspaceTitleUpdate::Manual {
                workspace_id,
                result,
            }) => self.finish_workspace_title_update(workspace_id, result, true, window, cx),
        ...
        match result {
            WorkspaceTitleResult::Updated(change) => {
                ...
                if active_workspace_updated {
                    self.rekey_workspace_session_after_title_change(new_workspace_id.clone());
                    if self
                        .settings_state
                        .rebind_graph_workspace_target_after_rename(
                            &old_workspace_id,
                            new_workspace_id,
                        )
                    {
    ...
    fn block_workspace_picker_transition_if_needed(
        ...
        let reason = workspace_picker::workspace_picker_transition_path_disabled_reason(
            ...
            workspace_picker::WorkspacePickerTransitionBlockers {
                edit_rollback_work: false,
                edit_replacement_work: false,
            },
        );
    ...
    fn finish_workspace_picker_opened_workspace(
        ...
        let loaded = LoadedWorkspaceState::new(
            ...
            opened.threaded_decision_state,
            None,
        );
        if loaded.selected_runtime().is_some() {
            self.state = ShellState::WorkspaceLoaded(loaded);
    ...
    fn finish_workspace_picker_deleted_workspace(
        ...
        if self.loaded_workspace().is_none() {
            self.settings_state.clear_graph_workspace_target();
            self.sync_settings_window_model(cx);
        }
    ...
    pub(super) fn activate_beryl_thread_link(
        ...
        let target = match self.activated_link_thread_target(&thread_id) {
            Ok(target) => target,
            Err((title, message)) => {
                if let Some(surface) = self.conversation_surface_mut() {
                    surface.set_notice(SurfaceNotice::new(title, message));
                }
                cx.notify();
                return false;
            }
    ...
    fn start_new_thread(&mut self, _: &gpui::ClickEvent, _: &mut Window, cx: &mut Context<Self>) {
        ...
        if updated {
            self.begin_unavailable_transcript_host_activation(
    ...
    fn copy_transcript_selection_action(
        &mut self,
        _: &CopyTranscriptSelection,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let payload = match self.transcript_panel.read(cx).resident_copy_payload() {
            Ok(payload) => payload,
            Err(_) => {
                self.transcript_panel
                    .read(cx)
                    .unavailable_command("copy_transcript_selection");
                return;
            }
        };

        let Some(text) = Self::transcript_clipboard_text_from_resident_payload(payload) else {
            self.transcript_panel
                .read(cx)
                .unavailable_command("copy_transcript_selection");
            return;
    ...
    fn transcript_clipboard_text_from_resident_payload(
        payload: syndic_transcript::ResidentTranscriptCopyPayload,
    ) -> Option<String> {
    ...
    fn open_transcript_context_menu_action(
        &mut self,
        _: &OpenTranscriptContextMenu,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let _ = self.open_transcript_context_menu_from_resident_target(cx);
    }
    ...
    fn open_transcript_context_menu_from_resident_target(
        ...
        {
            syndic_transcript::ResidentContextMenuCommandTarget::Targeted(target) => {
                Self::accept_resident_transcript_context_menu_target(target)
            }
            syndic_transcript::ResidentContextMenuCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("open_transcript_context_menu"),
        }
    }

    fn accept_resident_transcript_context_menu_target(
        target: syndic_transcript::ResidentTranscriptContextMenuTarget,
    ) -> syndic_transcript::TranscriptCommandResult {
    ...
    fn edit_resident_context_target_from_panel(
        ...
        match target {
            ...
            syndic_transcript::ResidentEditCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("edit_resident_context_target"),
        }
    }
    ...
    fn branch_resident_context_target_action(
        &mut self,
        _: &BranchResidentContextTarget,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let _ = self.branch_resident_context_target_from_panel(cx);
    }

    fn branch_resident_context_target_from_panel(
        &mut self,
        cx: &mut Context<Self>,
    ) -> syndic_transcript::TranscriptCommandResult {
        let target =
            syndic_transcript::ResidentBranchCommandTarget::from_context_menu_command_target(
                self.transcript_panel
                    .read(cx)
                    .resident_context_menu_command_target(),
            );
        match target {
            ...
            syndic_transcript::ResidentBranchCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("branch_resident_context_target"),
        }
    }
    ...
    fn preview_resident_transcript_media_action(
        &mut self,
        _: &PreviewResidentTranscriptMedia,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let _ = self.preview_resident_transcript_media_from_panel(cx);
    }
    ...
    fn preview_resident_transcript_media_from_panel(
        ...
        match self
            .transcript_panel
            .read(cx)
            .resident_media_preview_command_target()
        {
            syndic_transcript::ResidentMediaPreviewCommandTarget::Targeted(payload) => {
                Self::accept_resident_media_preview_payload(payload)
            }
            syndic_transcript::ResidentMediaPreviewCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("preview_resident_transcript_media"),
        }
    }

    fn accept_resident_media_preview_payload(
        payload: syndic_transcript::ResidentMediaPreviewCommandPayload,
    ) -> syndic_transcript::TranscriptCommandResult {
        let _record_ids = payload.record_ids();
        let _preview_range = payload.range();
        let _preview_byte_len = payload.byte_len();
        syndic_transcript::TranscriptCommandResult::NoOp
    }

    fn copy_resident_transcript_media_action(
        &mut self,
        _: &CopyResidentTranscriptMedia,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let _ = self.copy_resident_transcript_media_from_panel(cx);
    }

    fn copy_resident_transcript_media_from_panel(
        &mut self,
        cx: &mut Context<Self>,
    ) -> syndic_transcript::TranscriptCommandResult {
        match self
            .transcript_panel
            .read(cx)
            .resident_media_copy_command_target()
        {
            syndic_transcript::ResidentMediaCopyCommandTarget::Targeted(payload) => {
                self.write_resident_media_copy_payload_to_clipboard(payload, cx)
            }
            syndic_transcript::ResidentMediaCopyCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("copy_resident_transcript_media"),
        }
    }

    fn write_resident_media_copy_payload_to_clipboard(
        &mut self,
        payload: syndic_transcript::ResidentMediaCopyCommandPayload,
        cx: &mut Context<Self>,
    ) -> syndic_transcript::TranscriptCommandResult {
        let Some(format) = Self::resident_media_copy_image_format(&payload) else {
            return self
                .transcript_panel
                .read(cx)
                .unavailable_command("copy_resident_transcript_media");
        ...
        let _record_ids = payload.record_ids();
        let _copy_range = payload.range();
        let _copy_byte_len = payload.byte_len();
        let image = Image::from_bytes(format, payload.bytes().to_vec());
        cx.write_to_clipboard(ClipboardItem::new_image(&image));
        syndic_transcript::TranscriptCommandResult::NoOp
    }

    fn resident_media_copy_image_format(
        payload: &syndic_transcript::ResidentMediaCopyCommandPayload,
    ) -> Option<ImageFormat> {
        payload.media_type().and_then(ImageFormat::from_mime_type)
    }

    fn save_resident_transcript_media_action(
        &mut self,
        _: &SaveResidentTranscriptMedia,
        _: &mut Window,
        cx: &mut Context<Self>,
    ) {
        let _ = self.save_resident_transcript_media_without_destination(cx);
    }
    ...
    fn save_resident_transcript_media_without_destination(
        ...
        self.transcript_panel
            .read(cx)
            .unavailable_command("save_resident_transcript_media")
    }

    fn save_resident_transcript_media_to_path(
        &mut self,
        destination: PathBuf,
        cx: &mut Context<Self>,
    ) -> syndic_transcript::TranscriptCommandResult {
        let Some(destination) = Self::resident_media_save_destination(destination) else {
            return self
                .transcript_panel
                .read(cx)
                .unavailable_command("save_resident_transcript_media");
        ...
        match self
            .transcript_panel
            .read(cx)
            .resident_media_save_command_target()
        {
            syndic_transcript::ResidentMediaSaveCommandTarget::Targeted(payload) => {
                self.write_resident_media_save_payload_to_path(payload, destination, cx)
            }
            syndic_transcript::ResidentMediaSaveCommandTarget::Unavailable(_) => self
                .transcript_panel
                .read(cx)
                .unavailable_command("save_resident_transcript_media"),
        }
    ...
    fn write_resident_media_save_payload_to_path(
        &mut self,
        payload: syndic_transcript::ResidentMediaSaveCommandPayload,
        destination: syndic_transcript::ResidentMediaSaveDestination,
        cx: &mut Context<Self>,
    ) -> syndic_transcript::TranscriptCommandResult {
        if !payload.complete() {
            return self
        ...
        let _record_ids = payload.record_ids();
        let _resource_id = payload.resource_id();
        let _save_range = payload.range();
        let _save_byte_len = payload.byte_len();
        let _save_media_type = payload.media_type();
        if fs::write(destination.path(), payload.bytes()).is_err() {
            return self
                .transcript_panel
                .read(cx)
                .unavailable_command("save_resident_transcript_media");
    ...
    fn resident_media_save_destination(
        destination: PathBuf,
    ) -> Option<syndic_transcript::ResidentMediaSaveDestination> {
        let destination = syndic_transcript::ResidentMediaSaveDestination::new(destination).ok()?;
        if !Self::resident_media_save_destination_path_is_safe(destination.path()) {
            return None;
        }
    ...
    fn resident_media_save_destination_path_is_safe(destination: &Path) -> bool {
    ...
    fn insert_transcript_quote_into_draft(
        ...
        let payload = match self.transcript_panel.read(cx).resident_quote_payload() {
            Ok(payload) => payload,
            Err(_) => {
                self.transcript_panel
                    .read(cx)
                    .unavailable_command("quote_transcript_selection");
                return false;
            }
        };
        let quoted_markdown = payload.quoted_markdown;
        if quoted_markdown.is_empty() {
            self.transcript_panel
                .read(cx)
                .unavailable_command("quote_transcript_selection");
            return false;
        ...
        let changed = self.conversation_input.update(cx, |input, cx| {
            input.replace_selected_text(&quoted_markdown, cx)
        });
        self.sync_composer_draft_from_input(cx);
        if changed {
            cx.notify();
    ...
    fn browse_composer_history(
        ...
        if self.conversation_input.read(cx).has_marked_text()
            || self
                .conversation_surface()
                .is_some_and(|surface| surface.transcript_edit_mode_active())
        {
            return;
        }
    ...
    fn jump_transcript_turn_up_action(
        ...
        self.jump_transcript_turn("up", window, cx);
    }
    ...
    fn jump_transcript_turn_down_action(
        ...
        self.jump_transcript_turn("down", window, cx);
    }
    ...
    fn jump_transcript_turn(
        ...
        let _ = (direction, window);
        self.transcript_panel
            .read(cx)
            .unavailable_command("jump_transcript_turn");
    }
    ...
    fn queue_steering_fragments_for_next_turn(
        ...
        match &mut self.state {
            ShellState::Ready(ready) => {
                ...
                let title_mode = Self::thread_title_mode_for_user_submission_state(
                    &ready.loaded_workspace.workspace_state,
                    Some(thread_id.as_str()),
                    automatic_title_generation_allowed,
                );
    ...
    fn begin_pending_turn_input_queue_for_thread(&mut self, thread_id: &str) -> bool {
        ...
        let title_mode = queue.title_mode();
        let turn_options = self.turn_options_with_current_developer_instructions(
            selected_thread_id.as_deref(),
    ...
    fn closed_decision_branch_submission_block(&self) -> Option<BackendOperationBlock> {
        let thread_id = self
            .conversation_surface()
            .and_then(ConversationSurfaceState::selected_thread_id)?;
        self.thread_is_read_only_decision_branch(thread_id)
            .then(|| BackendOperationBlock {
            kind: "closed_decision_branch",
            message:
                "This is a closed decision branch. The transcript is read-only; continue from the parent handoff or start a superseding decision branch."
                    .to_string(),
        })
    }

    fn thread_is_read_only_decision_branch(&self, thread_id: &str) -> bool {
        let thread_id = ConversationThreadId::new(thread_id.to_string());
        self.workspace_shell_state().is_some_and(|loaded| {
            child_thread_is_read_only_decision_branch(&loaded.threaded_decision_state, &thread_id)
        })
    }
    ...
    fn finish_pending_decision_handoff_navigation_for_thread(
        &mut self,
        thread_id: &str,
        window: &mut Window,
        cx: &mut Context<Self>,
        ...
        let matches_parent = self
            .pending_decision_handoff_navigation
            .as_ref()
            .is_some_and(|pending| pending.parent_thread_id.as_str() == thread_id);
        if !matches_parent {
            return false;
        }
        let Some(pending) = self.pending_decision_handoff_navigation.take() else {
            return false;
        };
        ...
        if let Some(surface) = self.conversation_surface_mut() {
            surface.set_notice(SurfaceNotice::new(
                "Decision handoff opened",
                format!(
                    "Beryl opened the parent thread for decision record {}, but the recorded handoff turn {} is not loaded in the visible transcript history.",
    ...
    fn update_workspace_state_for_opened_target(
        ...
        if let Some(loaded) = self.workspace_shell_state_mut() {
            if let Some(active_thread_id) = active_thread_id
                && !active_thread_is_read_only_decision_branch
            {
    ...
    fn remember_thread_summary(
        ...
        let activated = activate
            && loaded
                ...
                .is_some();
        let should_persist = activated || touched_manifest;
    ...
    fn persist_current_threaded_decision_state(&mut self) {
        let Some((workspace_id, threaded_decision_state)) =
            self.workspace_shell_state().map(|loaded| {
                (
                    loaded.workspace.id().clone(),
                    loaded.threaded_decision_state.clone(),
                )
            })
        else {
            return;
        ...
        self.workspace_persistence_queue
            .save_threaded_decision_state(workspace_id, threaded_decision_state);
    }
    ...
    fn transcript_ui_state(&self, visible_row_limit: usize) -> TranscriptUiState {
        let _ = visible_row_limit;
        let visible_range = 0..0;
        let presentation_range = 0..0;

        TranscriptUiState {
            item_count: 0,
            visible_range: Some(ui_range_diagnostic(&visible_range)),
            presentation_range: Some(ui_range_diagnostic(&presentation_range)),
            scroll_position: transcript_scroll_position_diagnostic(),
            user_scrolled: false,
            pending_thread_activation_label: self
                .conversation_surface()
                .and_then(|surface| {
                    surface
                ...
                .map(bounded_control_string),
            pending_thread_activation_progress: self
                .conversation_surface()
                .and_then(ConversationSurfaceState::pending_thread_activation_progress),
            older_history_loading: false,
            visible_row_count: 0,
            visible_rows: Vec::new(),
            visible_rows_truncated: false,
        }
    }
    ...
    fn popup_ui_state(&self, cx: &mut Context<Self>) -> PopupUiState {
        ...
        PopupUiState {
            ...
            transcript_branch_menu_open: surface
                .is_some_and(|surface| surface.transcript_branch_menu_open()),
    ...
    fn apply_transcript_scroll_command(
        ...
        match arguments.command {
            ScrollTranscriptCommand::Wheel => {
                ...
                let manual_delta_px = -delta_y;
                self.transcript_panel.update(cx, |panel, cx| {
                    for _ in 0..arguments.repeat {
                        let _ = panel.manual_scroll_delta(viewport_height_px, manual_delta_px, cx);
                    }
                });
                Ok(())
            }
            ScrollTranscriptCommand::Top | ScrollTranscriptCommand::Bottom => {
                self.transcript_panel
                    .read(cx)
                    .unavailable_command("scroll_transcript");
                Err(
                    "Explicit transcript navigation is not ported to the resident host yet."
                        .to_string(),
                )
            }
        }
    ...
    fn handle_close_popups_tool_result(&mut self, cx: &mut Context<Self>) -> ClosePopupsResult {
        ...
        if let Some(surface) = self.conversation_surface_mut() {
            ...
            if surface.transcript_branch_menu_open() {
