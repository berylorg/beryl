...
- This crate applies selected-thread activation as one shell-state transaction: selected-thread chrome, transcript rows, and the initial transcript viewport are chosen before the newly activated transcript first renders. Renderer, prepaint, deferred, and post-frame callbacks must not install activation-owned transcript scroll changes after that first frame.
...
- Transcript live-turn anchoring and its virtual response runway are clients of this reusable scroll support; transcript code supplies user-fragment, commentary, and final-answer geometry but does not own a transcript-specific spacer row model. Loaded-history activation may choose a tail viewport before the transcript first renders and may expose passive final-answer trailing allowance for manual scrolling. If the latest loaded row gains final-answer geometry through historical detail replacement while still in tail activation or manual-detached state that came from tail activation, transcript code may refresh that passive allowance, but reusable scroll support must not be used to install a second activation-owned viewport or synthetic live-turn prompt or final anchor through render-time callbacks.
