...
- This crate uses exact non-empty multi-agent v2 `agentPath` values as child activity display labels and uses the fixed label `Subagent` for pathless legacy child activity. It does not enqueue activity-only background reads for backend-generated subagent nicknames. Exact child-thread model/reasoning metadata may still be applied from normalized activity events or already available thread metadata; missing metadata remains unknown rather than inferred.
