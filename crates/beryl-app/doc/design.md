...
- This crate uses exact non-empty multi-agent v2 `agentPath` values as child activity display labels and uses the fixed label `Subagent` for pathless legacy child activity. It does not enqueue activity-only background reads for backend-generated subagent nicknames. Exact child-thread model/reasoning metadata may still be applied from normalized activity events or genuinely read-only thread metadata; missing metadata remains unknown rather than inferred. The crate intentionally preserves its conditional child-label formatter and normalized metadata intake for future CAS protocol support, but it must not issue `thread/resume` or another activation or subscription operation solely to populate Activity labels.
...
- Activity display values, reasoning-summary fragments, and labels other than exact multi-agent v2 `agentPath` identities are presentation payloads that must be truncated at projection ingress before they can exceed deterministic memory budgets. Exact v2 paths remain verbatim, count toward completed-row byte pruning, and are clipped only by fixed-width row rendering; projection truncation must not modify backend-owned transcript history.
...
- The activity projection owns the complete `Agent` display value consumed by rendering. Activity agent labels use `Main` for the selected parent thread, exact non-empty v2 `agentPath` values for observed v2 child activity, `Subagent` for pathless legacy child activity, and known non-subagent labels only when those labels are real user-facing values rather than generated from backend ids. Child labels may append exact model/reasoning metadata when known. Parent and non-subagent labels do not receive child metadata suffixes. Backend thread ids remain internal correlation keys and never render as labels.
- The activity projection treats `agentPath` as presentation metadata only. Child ownership, nesting, and lifecycle correlation remain keyed by exact backend thread ids.
- A malformed v2 child record with a missing or blank required `agentPath` remains unlabeled and does not use the pathless legacy `Subagent` label.
- Running parent activity sorts first with the active `Main` row at the top. Running child activity follows in stable first-active order while each child remains active, compacting when earlier children finish and appending newly active children. Finished activity follows and sorts by newest completion first.
...
- Supported `outcome` values are `phase_needs_review`, `blocked_needs_operator`, `phase_continue`, `phase_continue_new_thread`, and `plan_complete`.
...
- `phase_continue_new_thread` records clean phase-thread continuation. It suppresses ordinary end-turn sound, does not compact the source, and may begin child preparation only after the exact source thread and turn complete successfully and the source turn worker clears its turn and dynamic-tool receivers. Completion matching requires both identities; a same-turn event from another thread or same-thread event from another turn cannot release the worker.
- The Beryl-generated resume fragment text for `phase_continue` and `phase_continue_new_thread` is `Continue from the root doc/plan.md.`.
- The `phase_continue` resume fragment participates in the same pending-turn input queue as composer fragments accepted while compaction is active, so all next-turn inputs preserve deterministic queue order. `phase_continue_new_thread` keeps conflicting composer input unavailable until child preparation and selection finish.
- Before `phase_continue_new_thread` preparation starts, this crate fails any already-accepted pending source-thread input through normal delivery-failure presentation. It does not start another source turn or move that input to the clean child.
...
- Yield lifecycle actions must not mark a turn complete locally, rewrite source transcript history, or treat compaction request acceptance as compaction completion. After the required source completion boundary, `phase_continue_new_thread` may mutate only a newly forked child's inherited backend history through the feature-defined full rollback.

### Clean Phase Thread Preparation

- This crate owns a dedicated lifecycle phase-thread preparation state and background worker. It must not overload selected-thread compaction, ordinary thread activation, or transcript branching state.
- The preparation request freezes an exact request generation, workspace id, source thread id, source turn id, orchestration-root thread id, execution target, canonical working directory, workspace-member binding, and source selection identity.
- The first source in a sequence establishes the orchestration root. A child produced by this lifecycle records that exact root as orchestration provenance for later sibling creation, while selector parentage continues to come only from verified backend `forkedFromId`.
- The worker uses an independent backend client for the exact source runtime, forks the orchestration root with inherited turns populated, rolls back every inherited user turn, then reads or validates the resulting child history and fork parent.
- A successful preparation result requires empty effective child history, `forkedFromId` equal to the orchestration root, and returned runtime/working-directory metadata consistent with the frozen execution target and member binding.
- Preparation results distinguish verified success, definitive failure before any child is known, failure with an exact known orphan child, and indeterminate fork commitment where timeout or transport loss may have created a child whose id was not returned.
- The shell keeps the source selected throughout preparation and exposes one lifecycle-preparation busy state. Thread selection, new-thread creation, branch/edit/history mutation, compaction, composer submission, steering, and turn-stop entry points must reject or render unavailable while that state is active.
- The shell may accept a preparation result only when its exact request generation and every frozen workspace, source, root, execution-target, and selection identity still match. Stale results never select a thread or start a turn.
- After accepting a fully prepared result, this crate registers the child as Beryl-created, selects the exact returned child id without rediscovering it through inventory, marks inventory refresh needed, and starts one ordinary top-level turn with the fixed continuation fragment.
- The child continuation uses normal request assembly and late-bound current hidden developer instructions. It uses normal title-generation eligibility and must not copy the root's title.
- A source turn whose backend terminal status is failed or interrupted, or whose worker fails or disconnects before orderly completion, clears the pending lifecycle request without forking.
- Definitive fork failure leaves the idle source selected and reports a bounded notice. Rollback or verification failure also reports the returned fork child as an orphan when it cannot be deleted and marks inventory refresh needed.
- An indeterminate fork result leaves the idle source selected, reports that an unidentified backend child may exist, and marks inventory refresh needed. This crate must not infer the child from inventory differences, register a guessed candidate, or activate any candidate automatically.
- If selection or workspace identity becomes stale after a child was prepared, this crate does not activate or start it. Any registration or inventory refresh remains scoped to the exact original workspace and reports the non-activation through a bounded notice.
- Late preparation results for a workspace that is no longer selected are retained as bounded workspace-scoped notices with any required inventory refresh, then applied only if that exact workspace becomes current again.
- If the fixed continuation turn fails to start after the prepared child was registered and selected, this crate leaves that child selected and idle, uses normal turn-delivery failure presentation, and does not retry automatically.
- Workspace replacement invalidates phase-thread activation authority before the replacement worker begins. Active-workspace deletion additionally waits for cancelling phase-thread preparation owned by that workspace to reach a definitive result or bounded indeterminate drain before capturing its persistence barrier and starting deletion; it then releases any deleted-workspace deferred outcomes so late registration cannot race deletion, recreate workspace storage, or retain unreplayable admission state. Application shutdown cancels and boundedly drains active and cancelling phase-thread preparation while its backend remains usable, applies definitive preparation outcomes, completes a final workspace-persistence flush, and only then shuts down managed backend servers.
- Child creation remains disabled until the targeted app-server contract proves that the forked-and-rolled-back child retains Beryl's registered dynamic tools and can call `yield` in a later turn. This crate must not replace that capability with an unrelated new thread or GUI-local lineage.

## Diagnostics And Diagnostic Child Control Tools

- This crate implements the process, protocol, bounds, cleanup, and evidence responsibilities assigned to `beryl-app` by `doc/systems/diagnostic-acceptance/design.md`.
...
- The public diagnostic-acceptance session facade accepts a typed launch mode. Fresh-workspace launch requires an explicit existing host workspace and passes it through Beryl's canonical `--host-path` startup option. Existing-home recovery requires the isolated Beryl home to exist, forbids a host-workspace input, and omits `--host-path` so startup restores persisted workspace state instead of registering a duplicate workspace. These mode-specific capabilities must not expand the `beryl_diagnostic.start` dynamic-tool schema or alter ordinary diagnostic-child launch defaults.
- The public diagnostic-acceptance session is host-Windows-only. Its internal launch adds a hidden startup gate: the target cannot enter application bootstrap until the supervisor has configured and assigned the exact kill-on-close Job Object, released the fixed stdin gate, and received the target's fixed pre-protocol ready frame. Ordinary diagnostic child startup remains ungated.
- Diagnostic protocol requests use one owned child-stdin writer and include enqueue, write, flush acknowledgement, and response receipt in the same deadline. Protocol failure and observed exit retain the exact child until explicit bounded shutdown or reap captures stderr and the actual termination method. Cleanup terminates the retained Job before joining owned transport so inherited pipe handles close, retains that Job through any incomplete join for retry, and releases it only after all joins succeed. Acceptance startup failures expose a stable public cause category while keeping supervisor error details private.
- The diagnostic-acceptance facade exposes a compiler for sequential diagnostic operations while keeping protocol framing and supervisor internals private. Compilation validates canonical command support and arguments, resolves the default or explicit timeout against configured and absolute maxima, and proves every possible encoded protocol frame fits the newline-inclusive frame bound before process launch. Compiler and protocol tests own exact frame-boundary proof; public acceptance plans prove launch-none rejection for their stricter plan-file and semantic payload bounds. The facade also validates Beryl-owned time, count, output, and path bounds before launch and owns exact-process cleanup through terminal evidence publication.
- The acceptance facade and diagnostic dynamic-tool boundary share one canonical `wait_for_state` implementation for argument normalization, predicates, identity guards, bounded polling, and result formatting. Acceptance executes that operation supervisor-side through repeated existing `read_ui_state` protocol requests; `wait_for_state` is not added to the child wire vocabulary and never blocks the child `gpui` thread.
- The facade distinguishes logical plan operations from expanded protocol requests. Its compiler derives each wait's exact worst-case poll count from normalized bounds, rejects a plan beyond the Beryl-owned expanded-request maximum, rejects summed worst-case operation budgets beyond total runtime, and qualifies frames with the plan-wide largest possible request id after handshake.
- Acceptance wait execution supplies the shared poller with the earliest normalized-wait, compiled-operation, and remaining-runtime deadline. The poller checks expiry before every request and never sends a zero-duration protocol request.
- If bounded acceptance startup cleanup expires, the facade returns a must-use owner-bearing failure with the original bounded error, exact process identity, and explicit bounded cleanup operation. It must not discard that owner through a text-only error or retry with ordinary supervisor timeouts.
- Consuming acceptance-session finish runs the configured cleanup attempt and, only when that remains indeterminate, at most one caller-budgeted recovery attempt before publication. It records both attempts, prevents session or supervisor destruction from starting another timed cleanup, and retains any still-indeterminate exact process owner in the must-use terminal outcome for non-waiting fail-safe release only.
- Acceptance evidence is versioned, bounded, metadata-oriented, and published without replacing an existing destination only after every finish-owned timed cleanup attempt. Frozen-executable byte count and digest derive from the same read stream. Schema v5 records the typed launch mode, represents the execution workspace only for fresh-workspace launch, and retains the optional exact contiguous protocol-request identity range for each logical operation, using first id, last id, and count so repeated wait polls remain attributable without an unbounded identity list. Evidence also records actual cleanup phase and method, bounded error, known retained identity, final verified or indeterminate residue, and publication outcome so cleanup failure, publication failure, and their combination stay distinguishable.
- Parameter bodies are retained only as byte counts and SHA-256 digests. Response, request-error, cleanup-error, and stderr payloads retain original byte counts, digests, UTF-8 prefix byte counts, and explicit truncation state. Their retained prefixes share the configured aggregate captured-output byte budget in deterministic run order; post-persist publication-failure text uses a separate fixed byte bound.
- Stderr hashing and total-byte accounting use original raw child bytes. Lossy UTF-8 conversion is only a presentation transform for the retained prefix and must account for encoded replacement bytes so invalid input cannot exceed the remaining evidence budget. This crate uses one qualified SHA-256 implementation for every acceptance digest and covers standard known-answer, block-boundary, and incremental-update vectors without adding an alternate shell or external hashing path.
...
- Inventory workers request backend-side working-directory filtering and newest-updated ordering, then enforce finite aggregate elapsed-time, page-count, result-count, and fork-parent metadata-read budgets across the refresh. They pass only the remaining elapsed-time budget to each backend request.
- A successful bounded listing may publish an explicitly truncated snapshot when another backend cursor remains. If at least one trustworthy page was accepted, later budget exhaustion or target/enrichment failure may publish one atomic partial snapshot with bounded incompleteness reasons. Failure before any trustworthy page preserves the last usable snapshot and publishes bounded failure state instead of replacing it with an empty successful snapshot.
- Fork-parent enrichment stops when its metadata-read or elapsed-time budget is exhausted. Rows left without backend parent metadata remain roots/orphans in the projection; this crate must not fill lineage from persisted registrations or lifecycle orchestration provenance.
...
- Inventory refresh may enrich backend thread-list summaries with metadata-only backend thread reads when list summaries do not include a durable fork parent id. This enrichment remains part of the off-thread refresh job and must publish one atomic complete or explicitly partial snapshot rather than making render-time backend calls.
...
- Persisted thread registrations and the selected conversation surface's `known_threads` cache do not populate thread-selector inventory rows. Selector rows remain backend-list-derived even when an exact registered thread has been reopened successfully.
...
- On workspace startup, one persisted active-thread registration may seed exact recovery only when its recorded execution target and member binding remain in scope and it is not marked rebind-required.
- Exact startup recovery uses the same direct metadata-resume and bounded history activation path as explicit activation. It does not wait for member-thread inventory, add registration-derived selector rows, or choose a different listed thread when exact validation fails.
- For a lifecycle phase child whose persisted orchestration root differs from its own id, exact recovery additionally validates backend `forkedFromId` against that root. Missing or contradictory backend lineage fails without GUI-local substitution; an orchestration root itself does not require self-parentage.
- Startup recovery freezes the workspace id, selected registration fingerprint, execution target, canonical working directory, member binding, and backend operation generation. Completion may select only when every frozen identity still matches.
- Successful direct recovery inserts only the backend-validated selected summary into the bounded selected-surface cache. A persisted identity that is missing binding metadata, binding-mismatched, out of scope, unavailable, or rebind-required remains preserved for explicit repair but is not presented as selected. Preflight-invalid identities issue neither list nor activation, leave no substitute selected, and produce a bounded repair notice. Only the absence of a persisted active identity permits ordinary bounded startup discovery.
...
- This crate also applies the active non-empty global developer-instructions preference as hidden developer-instructions context when it starts an automatic continuation turn produced by `yield(phase_continue)` or `yield(phase_continue_new_thread)`.
