...
- This crate owns transcript viewport defaults for loaded thread history, including opening existing threads at the tail of the loaded history window as a one-shot selected-thread-activation decision without synthesizing submit-time trailing scroll allowance for already-loaded prompts.
- Submit-time transcript anchoring is reserved for accepted user-authored turns and streaming responses. Loaded selected-thread history must not synthesize a passive submit anchor or deferred renderer-installed anchor to correct its activation viewport after paint.
...
- Transcript submit anchoring and its trailing allowance are clients of this reusable scroll support; transcript code supplies user-fragment line geometry but does not own a transcript-specific spacer row model. Loaded-history activation may choose a tail viewport before the transcript first renders, but reusable scroll support must not be used to install a second activation-owned viewport or synthetic submit anchor through render-time callbacks.
